...
class Gripper(GPIO):
    ...
    def gripper_is_support_status(self):
        if (
            self.gripper_version_numbers[0] == -1
            or self.gripper_version_numbers[1] == -1
            or self.gripper_version_numbers[2] == -1
        ):
            self.get_gripper_version()
        return (
            self.gripper_version_numbers[0] > 3
            or (
                self.gripper_version_numbers[0] == 3
                and self.gripper_version_numbers[1] > 4
            )
            or (
                self.gripper_version_numbers[0] == 3
                and self.gripper_version_numbers[1] == 4
                and self.gripper_version_numbers[2] >= 3
            )
        )
    ...
    def set_gripper_status(self, status, delay_sec=0):
        ...
        self.log_api_info(
            "API -> set_gripper_status(status={}, delay_sec={}) -> code={}".format(
                status, delay_sec, code
            ),
            code=code,
        )
        return code
    ...
    def _set_gripper_enable(self, enable):
        ...
        self.log_api_info(
            "API -> set_gripper_enable(enable={}) -> code={}".format(enable, ret[0]),
            code=ret[0],
        )
        return ret[0]
    ...
    def _set_gripper_mode(self, mode):
        ...
        self.log_api_info(
            "API -> set_gripper_mode(mode={}) -> code={}".format(mode, ret[0]),
            code=ret[0],
        )
        return ret[0]
    ...
    def _set_gripper_speed(self, speed):
        ...
        self.log_api_info(
            "API -> set_gripper_speed(speed={}) -> code={}".format(speed, ret[0]),
            code=ret[0],
        )
        return ret[0]
    ...
    def _set_gripper_position(
        self, pos, wait=False, speed=None, auto_enable=False, timeout=None, **kwargs
    ):
        if kwargs.get("wait_motion", True):
            ...
            if not (
                code == 0
                or (is_stop and code == APIState.EMERGENCY_STOP)
        ...
        self.log_api_info(
            "API -> set_gripper_position(pos={}) -> code={}".format(pos, ret[0]),
            code=ret[0],
        )
        if wait:
            is_add = True
    ...
    def _get_gripper_err_code(self):
        ...
        if self._check_code(ret[0]) == 0:
            ...
            if ret[0] == XCONF.UxbusState.ERR_CODE:
                ...
                if self.error_code == 19 or self.error_code == 28:
                    print(
                        "gripper/tgpio error, code=C{}, G{}".format(
                            self.error_code, ret[1]
                        )
                    )
                    return ret[0], ret[1]
            ret[0] = 0
    ...
    def _clean_gripper_error(self):
        ...
        self.log_api_info(
            "API -> clean_gripper_error -> code={}".format(ret[0]), code=ret[0]
        )
        return ret[0]
    ...
    def _set_gripper_zero(self):
        ...
        self.log_api_info(
            "API -> set_gripper_zero -> code={}".format(ret[0]), code=ret[0]
        )
        return ret[0]
    ...
    def _set_modbus_gripper_enable(self, enable):
        ...
        self.log_api_info(
            "API -> set_modbus_gripper_enable(enable={}) -> code={}, code2={}, err={}".format(
                enable, ret[0], _, err
            ),
            code=ret[0],
        )
        ...
        return (
            ret[0] if self._gripper_error_code == 0 else APIState.END_EFFECTOR_HAS_FAULT
        )
    ...
    def _set_modbus_gripper_mode(self, mode):
        ...
        _, err = self._get_modbus_gripper_err_code()
        self.log_api_info(
            "API -> set_modbus_gripper_mode(mode={}) -> code={}, code2={}, err={}".format(
                mode, ret[0], _, err
            ),
            code=ret[0],
        )
    ...
    def _set_modbus_gripper_speed(self, speed):
        ...
        _, err = self._get_modbus_gripper_err_code()
        self.log_api_info(
            "API -> set_modbus_gripper_speed(speed={}) -> code={}, code2={}, err={}".format(
                speed, ret[0], _, err
            ),
            code=ret[0],
        )
    ...
    @xarm_is_connected(_type="set")
    def _set_modbus_gripper_position(
        self, pos, wait=False, speed=None, auto_enable=False, timeout=None, **kwargs
    ):
        if kwargs.get("wait_motion", True):
            has_error = self.error_code != 0
        ...
        self.log_api_info(
            "API -> set_modbus_gripper_position(pos={}) -> code={}".format(pos, ret[0]),
            code=ret[0],
        )
        _, err = self._get_modbus_gripper_err_code()
        if self._gripper_error_code != 0:
    ...
    def _clean_modbus_gripper_error(self):
        ...
        _, err = self._get_modbus_gripper_err_code()
        self.log_api_info(
            "API -> clean_modbus_gripper_error -> code={}, code2={}, err={}".format(
                ret[0], _, err
            ),
            code=ret[0],
        )
    ...
    def _set_modbus_gripper_zero(self):
        ...
        _, err = self._get_modbus_gripper_err_code()
        self.log_api_info(
            "API -> set_modbus_gripper_zero -> code={}, code2={}, err={}".format(
                ret[0], _, err
            ),
            code=ret[0],
        )
    ...
    def __bio_gripper_send_modbus(self, data_frame, min_res_len=0):
        ...
        if code != 0:
            return code, []
        return self.getset_tgpio_modbus_data(
            data_frame, min_res_len=min_res_len, ignore_log=True
        )
    ...
    def __bio_gripper_wait_motion_completed(self, timeout=5, **kwargs):
        ...
        while time.monotonic() < expired:
            ...
            if _ == 0:
                code = (
                    code
                    if (status & 0x03) == XCONF.BioGripperState.IS_MOTION
                    else (
                        APIState.END_EFFECTOR_HAS_FAULT
                        if (status & 0x03) == XCONF.BioGripperState.IS_FAULT
                        else (
                            0
                            if not check_detected
                            or (status & 0x03) == XCONF.BioGripperState.IS_DETECTED
                            else code
                        )
                    )
                )
                ...
                code = (
                    ...
                    if _ == APIState.NOT_CONNECTED
                    else APIState.CHECK_FAILED if failed_cnt > 10 else code
                )
    ...
    def check_bio_gripper_is_catch(self, timeout=3):
        return (
            self.__bio_gripper_wait_motion_completed(
                timeout=timeout, check_detected=True
            )
            == 0
        )
    ...
    def set_bio_gripper_enable(self, enable, wait=True, timeout=3):
        ...
        self.log_api_info(
            "API -> set_bio_gripper_enable(enable={}, wait={}, timeout={}) ->code={}".format(
                enable, wait, timeout, code
            ),
            code=code,
        )
    ...
    def set_bio_gripper_speed(self, speed):
        ...
        code, _ = self.__bio_gripper_send_modbus(data_frame, 6)
        self.log_api_info(
            "API -> set_bio_gripper_speed(speed={}) ->code={}".format(speed, code),
            code=code,
        )
    ...
    def set_bio_gripper_position(self, pos, speed=0, wait=True, timeout=5, **kwargs):
        ...
        self.log_api_info(
            "API -> set_bio_gripper_position(pos={}, wait={}, timeout={}) ->code={}".format(
                pos, wait, timeout, code
            ),
            code=code,
        )
        return code
    ...
    def open_bio_gripper(self, speed=0, wait=True, timeout=5, **kwargs):
        return self.set_bio_gripper_position(
            130, speed=speed, wait=wait, timeout=timeout, **kwargs
        )
        # if kwargs.get('auto_enable', False) and not self.bio_gripper_is_enabled:
        #     self.set_bio_gripper_enable(True)
        # if speed > 0 and speed != self.bio_gripper_speed:
    ...
    def close_bio_gripper(self, speed=0, wait=True, timeout=5, **kwargs):
        return self.set_bio_gripper_position(
            50, speed=speed, wait=wait, timeout=timeout, **kwargs
        )
    ...
    def get_bio_gripper_version(self):
        code, ret = self.get_bio_gripper_register(
            address=0x0801, number_of_registers=0x03
        )
        versions = ["*", "*", "*"]
        if code == 0:
    ...
    def clean_bio_gripper_error(self):
        ...
        code, _ = self.__bio_gripper_send_modbus(data_frame, 6)
        self.log_api_info(
            "API -> clean_bio_gripper_error -> code={}".format(code), code=code
        )
