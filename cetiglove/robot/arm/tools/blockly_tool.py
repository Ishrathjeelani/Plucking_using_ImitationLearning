...
class BlocklyToolOld(object):
    ...
    def _init_py3(
        ...
        self._insert_to_file(
            self.index, "# Software License Agreement (BSD License)\n#"
        )
        self._insert_to_file(
            self.index,
            "# Copyright (c) {}, UFACTORY, Inc.".format(
        ...
        self._insert_to_file(
            self.index, "#   1. Changes to this file on Studio will not be preserved"
        )
        self._insert_to_file(
            self.index,
        ...
        self._insert_to_file(
            self.index, "# git clone git@github.com:xArm-Developer/xArm-Python-SDK.git"
        )
        ...
        self._insert_to_file(self.index, "    try:")
        self._insert_to_file(
            self.index, "        stack_tuple = traceback.extract_stack(limit=2)[0]"
        )
        self._insert_to_file(
            self.index,
            "        print('[{}][{}] {}'.format("
        ...
        if arm is None:
            self._insert_to_file(
                self.index, "arm = XArmAPI(sys.argv[1], baud_checkset=False)"
            )
        elif isinstance(arm, str):
            self._insert_to_file(
                self.index, "arm = XArmAPI('{}', baud_checkset=False)".format(arm)
            )
        if init:
        ...
        if error_exit:
            ...
            self._insert_to_file(
                self.index, "        pprint('err={}, quit'.format(data['error_code']))"
            )
            self._insert_to_file(
                self.index,
            ...
            for block in blocks:
                ...
                while block is not None:
                    ...
                    if block.attrib.get("disabled", False):
                        continue
                    func = getattr(
                        self, "_handle_{}".format(block.attrib["type"]), None
                    )
                    if func:
                        if self._highlight_callback is not None:
                            if block.attrib["type"] in HIGHLIGHT_BLOCKS:
                                self._append_to_file(
                                    "{}if not params['quit']:".format(prefix)
                                )
                                self._append_to_file(
                                    "{}    highlight_callback('{}')".format(
                                        prefix, block.attrib["id"]
                                    )
                                )
                        ...
                        self._succeed = False
                        print(
                            "block {} can't convert to python code".format(
                                block.attrib["type"]
                            )
                        )
    ...
    def _handle_move(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file(
            "{}    code = arm.set_position({}={}, speed=params['speed'], mvacc=params['acc'], "
        ...
        self._append_to_file(
            "{}        pprint('set_position, code={{}}'.format(code))".format(prefix)
        )
    ...
    def _handle_move_arc_to(self, block, prefix="", arg_map=None):
        ...
        if self._show_comment:
            self._append_to_file(
                "{}# move{}line and {}".format(
                    prefix,
                    " arc " if float(radius) >= 0 else " ",
    ...
    def _handle_move_circle(self, block, prefix="", arg_map=None):
        ...
        if self._show_comment:
            self._append_to_file(
                "{}# move circle and {}".format(prefix, "wait" if wait else "no wait")
            )
        self._append_to_file(
            "{}if arm.error_code == 0 and not params['quit']:".format(prefix)
        )
        self._append_to_file(
            "{}    code = arm.move_circle({}, {}, float({}) / 360 * 100, speed=params['speed'], mvacc=params['acc'], "
            "wait={})".format(prefix, pose1, pose2, percent, wait)
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('move_circle, code={{}}'.format(code))".format(prefix)
        )
    ...
    def _handle_move_7(self, block, prefix="", arg_map=None):
        ...
        if self._show_comment:
            self._append_to_file(
                "{}# move joint and {}".format(prefix, "wait" if wait else "no wait")
            )
        self._append_to_file(
            "{}if arm.error_code == 0 and not params['quit']:".format(prefix)
        )
        self._append_to_file(
            "{}    code = arm.set_servo_angle(angle={}, speed=params['angle_speed'], "
            "mvacc=params['angle_acc'], wait={})".format(prefix, values, wait)
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('set_servo_angle, code={{}}'.format(code))".format(prefix)
        )
    ...
    def _handle_move_joints(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file(
            "{}    code = arm.set_servo_angle(angle={}, speed=params['angle_speed'], "
            "mvacc=params['angle_acc'], wait={}, radius={})".format(
                prefix, values, wait, radius
            )
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
    ...
    def _handle_move_tool_line(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file(
            "{}    code = arm.set_tool_position(*{}, speed=params['speed'], mvacc=params['acc'], "
            "wait={})".format(prefix, values, wait)
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('set_tool_position, code={{}}'.format(code))".format(
                prefix
            )
        )
    ...
    def _handle_studio_run_traj(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file(
            "{}    code = arm.playback_trajectory(times={}, filename='{}', wait=True)".format(
                prefix, times, filename
            )
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('playback_trajectory, code={{}}'.format(code))".format(
                prefix
            )
        )
    ...
    def _handle_app_studio_traj(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file(
            "{}    code = arm.playback_trajectory(times={}, filename='{}', wait=True, double_speed={})".format(
                prefix, times, filename, speed
            )
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
    ...
    def _handle_tool_console_with_variable(self, block, prefix="", arg_map=None):
        ...
        if msg:
            self._append_to_file(
                "{}print({}.format({}))".format(
                    prefix, json.dumps(msg + "{}", ensure_ascii=False), expression
                )
            )
            # self._append_to_file('{}pprint(\'{}{{}}\'.format({}))'.format(prefix, msg, expression))
        else:
            self._append_to_file(
                "{}print('{{}}'.format({}))".format(prefix, expression)
            )
    ...
    def _handle_gpio_set_digital(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file(
            "{}    code = arm.set_tgpio_digital({}, {}, delay_sec={})".format(
                prefix, io, value, delay_sec
            )
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('set_tgpio_digital, code={{}}'.format(code))".format(
                prefix
            )
        )
    ...
    def _handle_gpio_set_digital_with_xyz(self, block, prefix="", arg_map=None):
        ...
        if self._show_comment:
            self._append_to_file(
                "{}# set tgpio-{} digital with pos {}".format(prefix, io, xyz)
            )
        self._append_to_file(
            "{}if arm.error_code == 0 and not params['quit']:".format(prefix)
        )
        self._append_to_file(
            "{}    code = arm.set_tgpio_digital_with_xyz({}, {}, {}, {})".format(
                prefix, io, value, xyz, tol_r
            )
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('set_tgpio_digital_with_xyz, code={{}}'.format(code))".format(
                prefix
            )
        )
    ...
    def _handle_check_air_pump_state(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file("{}if not params['quit']:".format(prefix))
        self._append_to_file(
            "{}    arm.arm.check_air_pump_state({}, timeout={})".format(
                prefix, state, timeout
            )
        )
    ...
    def _handle_check_bio_gripper_is_catch(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file("{}if not params['quit']:".format(prefix))
        self._append_to_file(
            "{}    arm.arm.check_bio_gripper_is_catch(timeout={})".format(
                prefix, timeout
            )
        )
    ...
    def _handle_check_robotiq_is_catch(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file("{}if not params['quit']:".format(prefix))
        self._append_to_file(
            "{}    arm.arm.check_robotiq_is_catch(timeout={})".format(prefix, timeout)
        )
    ...
    def _handle_set_suction_cup(self, block, prefix="", arg_map=None):
        ...
        if self._show_comment:
            self._append_to_file(
                "{}# set_suction_cup({}, wait={}, delay_sec={})".format(
                    prefix, on, wait, delay_sec
                )
            )
        self._append_to_file(
            "{}if arm.error_code == 0 and not params['quit']:".format(prefix)
        ...
        self._append_to_file(
            "{}    code = arm.set_suction_cup({}, wait={}, delay_sec={})".format(
                prefix, on, wait, delay_sec
            )
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('set_suction_cup, code={{}}'.format(code))".format(prefix)
        )
    ...
    def _handle_gpio_set_controller_digital(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file(
            "{}    code = arm.set_cgpio_digital({}, {}, delay_sec={})".format(
                prefix, io, value, delay_sec
            )
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('set_cgpio_digital, code={{}}'.format(code))".format(
                prefix
            )
        )

    def _handle_gpio_set_controller_digital_with_xyz(
        self, block, prefix="", arg_map=None
    ):
        fields = self.get_nodes("field", root=block)
        x = fields[0].text
        y = fields[1].text
        ...
        if self._show_comment:
            self._append_to_file(
                "{}# set cgpio-{} digital with pos {}".format(prefix, io, xyz)
            )
        self._append_to_file(
            "{}if arm.error_code == 0 and not params['quit']:".format(prefix)
        )
        self._append_to_file(
            "{}    code = arm.set_cgpio_digital_with_xyz({}, {}, {}, {})".format(
                prefix, io, value, xyz, tol_r
            )
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('set_cgpio_digital_with_xyz, code={{}}'.format(code))".format(
                prefix
            )
        )
    ...
    def _handle_gpio_set_controller_digital_with_xyz_do(
        self, block, prefix="", arg_map=None
    ):
        fields = self.get_nodes("field", root=block)
        x = fields[0].text
    ...
    def _handle_gpio_set_controller_analog_with_xyz(
        self, block, prefix="", arg_map=None
    ):
        fields = self.get_nodes("field", root=block)
        x = fields[0].text
        ...
        if self._show_comment:
            self._append_to_file(
                "{}# set cgpio-{} analog with pos {}".format(prefix, io, xyz)
            )
        self._append_to_file(
            "{}if arm.error_code == 0 and not params['quit']:".format(prefix)
        )
        self._append_to_file(
            "{}    code = arm.set_cgpio_analog_with_xyz({}, {}, {}, {})".format(
                prefix, io, value, xyz, tol_r
            )
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('set_cgpio_analog_with_xyz, code={{}}'.format(code))".format(
                prefix
            )
        )
    ...
    def _handle_gpio_set_controller_analog(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file(
            "{}    code = arm.set_cgpio_analog({}, {})".format(prefix, io, value)
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('set_cgpio_analog, code={{}}'.format(code))".format(
                prefix
            )
        )
    ...
    def _handle_set_collision_sensitivity(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file("{}if not params['quit']:".format(prefix))
        self._append_to_file(
            "{}    arm.set_collision_sensitivity({})".format(prefix, value)
        )
    ...
    def _handle_set_teach_sensitivity(self, block, prefix=""):
        ...
        self._append_to_file("{}if not params['quit']:".format(prefix))
        self._append_to_file(
            "{}    arm.set_teach_sensitivity({})".format(prefix, value)
        )
    ...
    def _handle_set_tcp_load(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file("{}if not params['quit']:".format(prefix))
        self._append_to_file(
            "{}    arm.set_tcp_load({}, [{}, {}, {}])".format(prefix, weight, x, y, z)
        )
    ...
    def _handle_set_gravity_direction(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file("{}if not params['quit']:".format(prefix))
        self._append_to_file(
            "{}    arm.set_gravity_direction([{}, {}, {}])".format(prefix, x, y, z)
        )
    ...
    def _handle_set_tcp_offset(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file("{}if not params['quit']:".format(prefix))
        self._append_to_file(
            "{}    arm.set_tcp_offset([{}, {}, {}, {}, {}, {}], wait=True)".format(
    ...
    def _handle_gripper_set(self, block, prefix="", arg_map=None):
        ...
        if fields is not None and len(fields) >= 3:
            ...
            wait = (
                self.get_nodes("field", root=values[2], descendant=True)[0].text
                == "TRUE"
            )
        if self._show_comment:
            self._append_to_file(
                "{}# set gripper position and {}".format(
                    prefix, "wait" if wait else "no wait"
                )
            )
        self._append_to_file(
        ...
        self._append_to_file(
            "{}    code = arm.set_gripper_position({}, wait={}, speed={}, auto_enable=True)".format(
                prefix, pos, wait, speed
            )
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('set_gripper_position, code={{}}'.format(code))".format(
                prefix
            )
        )
    ...
    def _handle_gripper_set_status(self, block, prefix="", arg_map=None):
        ...
        if self._show_comment:
            self._append_to_file(
                "{}# set_gripper_status({}, delay_sec={})".format(
                    prefix, status, delay_sec
                )
            )
        self._append_to_file(
            "{}if arm.error_code == 0 and not params['quit']:".format(prefix)
        ...
        self._append_to_file(
            "{}    code = arm._arm.set_gripper_status({}, delay_sec={})".format(
                prefix, status, delay_sec
            )
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('set_gripper_status, code={{}}'.format(code))".format(
                prefix
            )
        )
    ...
    def _handle_set_bio_gripper_init(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file(
            "{}    code = arm.set_bio_gripper_enable(True)".format(prefix)
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('set_bio_gripper_enable, code={{}}'.format(code))".format(
                prefix
            )
        )
    ...
    def _handle_set_bio_gripper(self, block, prefix="", arg_map=None):
        ...
        if on:
            if self._show_comment:
                self._append_to_file(
                    "{}# open_bio_gripper(speed={}, wait={})".format(
                        prefix, speed, wait
                    )
                )
            self._append_to_file(
            ...
            self._append_to_file(
                "{}    code = arm.open_bio_gripper(speed={}, wait={})".format(
                    prefix, speed, wait
                )
            )
            self._append_to_file("{}    if code != 0:".format(prefix))
            self._append_to_file("{}        params['quit'] = True".format(prefix))
            self._append_to_file(
                "{}        pprint('open_bio_gripper, code={{}}'.format(code))".format(
                    prefix
                )
            )
        else:
            if self._show_comment:
                self._append_to_file(
                    "{}# close_bio_gripper(speed={}, wait={})".format(
                        prefix, speed, wait
                    )
                )
            self._append_to_file(
            ...
            self._append_to_file(
                "{}    code = arm.close_bio_gripper(speed={}, wait={})".format(
                    prefix, speed, wait
                )
            )
            self._append_to_file("{}    if code != 0:".format(prefix))
            self._append_to_file("{}        params['quit'] = True".format(prefix))
            self._append_to_file(
                "{}        pprint('close_bio_gripper, code={{}}'.format(code))".format(
                    prefix
                )
            )
    ...
    def _handle_set_robotiq_init(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('robotiq_reset, code={{}}'.format(code))".format(prefix)
        )
        ...
        self._append_to_file(
            "{}    code, _ = arm.robotiq_set_activate(wait=True)".format(prefix)
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('robotiq_set_activate, code={{}}'.format(code))".format(
                prefix
            )
        )
    ...
    def _handle_set_robotiq_gripper(self, block, prefix="", arg_map=None):
        ...
        if self._show_comment:
            self._append_to_file(
                "{}# robotiq_set_position({}, speed={}, force={}, wait={})".format(
                    prefix, pos, speed, force, wait
                )
            )
        self._append_to_file(
            "{}if arm.error_code == 0 and not params['quit']:".format(prefix)
        ...
        self._append_to_file(
            "{}    code, _ = arm.robotiq_set_position({}, speed={}, force={}, wait={})".format(
                prefix, pos, speed, force, wait
            )
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('robotiq_set_position, code={{}}'.format(code))".format(
                prefix
            )
        )
    ...
    def __handle_gpio_event(self, gpio_type, block, prefix="", arg_map=None):
        if gpio_type.startswith("listen"):
            if gpio_type == "listen_tgpio_digital":
                self._append_to_file(
                    "\n{}params['events']['gpio'].listen_tgpio_digital = True".format(
                        prefix
                    )
                )
            elif gpio_type == "listen_tgpio_analog":
                self._append_to_file(
                    "\n{}params['events']['gpio'].listen_tgpio_analog = True".format(
                        prefix
                    )
                )
            elif gpio_type == "listen_cgpio_state":
                self._append_to_file(
                    "\n{}params['events']['gpio'].listen_cgpio_state = True".format(
                        prefix
                    )
                )
            else:
            ...
            if gpio_type == "tgpio_digital":
                ...
                self._append_to_file(
                    "\n\n{}# Define Tool GPIO-{} DIGITAL is changed callback".format(
                        prefix, io
                    )
                )
                ...
                self._append_to_file(
                    "\n\n{}# Define Tool GPIO-{} ANALOG is changed callback".format(
                        prefix, io
                    )
                )
            elif gpio_type == "cgpio_digital":
                name = "controller_gpio_{}_digital_is_changed_callback_{}".format(
                    io, num
                )
                self._append_to_file(
                    "\n\n{}# Define Contoller GPIO-{} DIGITAL is {} callback".format(
                        prefix, io, trigger
                    )
                )
                ...
                self._append_to_file(
                    "\n\n{}# Define Contoller GPIO-{} ANALOG is changed callback".format(
                        prefix, io
                    )
                )
            else:
            ...
            if statement:
                ...
                self._append_to_file(
                    "{}_callback() if not params['callback_in_thread'] else threading.Thread(target=_callback, daemon=True).start()".format(
                        prefix
                    )
                )
            else:
            ...
            if gpio_type == "tgpio_digital":
                ...
                return
        self._append_to_file(
            "{}if not params['events']['gpio'].alive:".format(old_prefix)
        )
        self._append_to_file(
            "{}    params['events']['gpio'].start()".format(old_prefix)
        )
        ...
        if "gpio" not in self._events:
            ...
            self._insert_to_file(
                self.index, "class {}(threading.Thread):".format(name2)
            )
            self._insert_to_file(
                self.index,
            ...
            self._insert_to_file(
                self.index, "        self.is_init_tgpio_digital = False"
            )
            self._insert_to_file(
                self.index, "        self.is_init_tgpio_analog = False"
            )
            self._insert_to_file(self.index, "        self.is_init_cgpio_state = False")
            self._insert_to_file(
                self.index, "        self.listen_tgpio_digital = False"
            )
            ...
            self._insert_to_file(
                self.index, "        self.tgpio_digital_callbacks = []"
            )
            ...
            self._insert_to_file(
                self.index, "                _, values = arm.get_tgpio_digital()"
            )
            self._insert_to_file(self.index, "                if _ == 0:")
            self._insert_to_file(
                self.index, "                    if self.is_init_tgpio_digital:"
            )
            self._insert_to_file(
                self.index,
            ...
            self._insert_to_file(
                self.index, "                    self.is_init_tgpio_digital = True"
            )
            ...
            self._insert_to_file(
                self.index, "                _, values = arm.get_tgpio_analog()"
            )
            self._insert_to_file(self.index, "                if _ == 0:")
            self._insert_to_file(
                self.index, "                    if self.is_init_tgpio_analog:"
            )
            self._insert_to_file(
                self.index,
            ...
            self._insert_to_file(
                self.index, "                    self.is_init_tgpio_analog = True"
            )
            ...
            self._insert_to_file(
                self.index, "                _, values = arm.get_cgpio_state()"
            )
            self._insert_to_file(self.index, "                if _ == 0:")
            self._insert_to_file(
                self.index,
            ...
            self._insert_to_file(
                self.index, "                    analogs = [values[6], values[7]]"
            )
            self._insert_to_file(
                self.index, "                    analogs_o = [values[8], values[9]]"
            )
            self._insert_to_file(
                self.index, "                    if self.is_init_cgpio_state:"
            )
            self._insert_to_file(
                self.index, "                        for item in self.cgpio_callbacks:"
            )
            self._insert_to_file(
                self.index, "                            if item['type'] == 'digital':"
            )
            self._insert_to_file(
                self.index,
            ...
            self._insert_to_file(
                self.index, "                            elif item['type'] == 'analog':"
            )
            self._insert_to_file(
                self.index, "                                for io in range(2):"
            )
            ...
            self._insert_to_file(
                self.index, "                    self.is_init_cgpio_state = True"
            )
    ...
    def _handle_procedures_defnoreturn(self, block, prefix="", arg_map=None):
        ...
        try:
            ...
            if not args:
                ...
                arg_map_ = {
                    arg: "arg_{}".format(i + 1) for i, arg in enumerate(arg_list)
                }
                self._append_to_file(
                    "    def {}(cls, {}):".format(
                        name, ",".join(map(lambda x: arg_map_[x], arg_list))
                    )
                )
            # self._append_to_file('    def {}(cls):'.format(name))
            prefix = "        "
        ...
        if args and values and len(args) == len(values):
            self._append_to_file(
                "{}MyDef.{}({})".format(
                    ...
                    ",".join(
                        [
                            self.__get_condition_expression(val, arg_map=arg_map)
                            for val in values
                        ]
                    ),
                )
    ...
    def _handle_variables_set(self, block, prefix="", arg_map=None):
        ...
        if arg_map and field in arg_map:
            ...
            self._append_to_file(
                "{}params['variables']['{}'] = {}".format(prefix, field, expression)
            )
    ...
    def _handle_math_change(self, block, prefix="", arg_map=None):
        ...
        if arg_map and field in arg_map:
            ...
            self._append_to_file(
                "{}params['variables']['{}'] += {}".format(prefix, field, val)
            )
        # self._append_to_file('{}if \'{}\' not in locals_keys and \'{}\' in locals():'.format(prefix, field, field))
    ...
    def _handle_controls_repeat_ext(self, block, prefix="", arg_map=None):
        ...
        if statement:
            ...
            if self._highlight_callback:
                self._append_to_file(
                    "{}interval = time.monotonic() - t1".format(prefix)
                )
                self._append_to_file("{}if interval < 0.001:".format(prefix))
                self._append_to_file(
                    "{}    time.sleep(0.001 - interval)".format(prefix)
                )
        else:
            self._append_to_file("{}pass".format(prefix))
    ...
    def _handle_controls_whileUntil(self, block, prefix="", arg_map=None):
        ...
        if field.text == "WHILE":
            ...
            expression = self.__get_condition_expression(value, arg_map=arg_map)
            self._append_to_file(
                "{}while {} and not params['quit']:".format(prefix, expression)
            )
        elif field.text == "UNTIL":
            value = self.get_node("value", root=block)
            expression = self.__get_condition_expression(value, arg_map=arg_map)
            self._append_to_file(
                "{}while not {} and not params['quit']:".format(prefix, expression)
            )
        prefix = "    " + prefix
        statement = self.get_node("statement", root=block)
        if statement:
    ...
    def _handle_controls_if(self, block, prefix="", arg_map=None):
        ...
        for st in statements:
            if st.attrib["name"] == "ELSE":
                ...
                self.parse(
                    st,
                    old_prefix if not has_if else "    " + old_prefix,
                    arg_map=arg_map,
                )
                break
    ...
    def __get_condition_expression(self, value_block, arg_map=None):
        ...
        if block.attrib["type"] == "logic_boolean":
            ...
            value = self.get_node("value", root=block)
            return "not ({})".format(
                self.__get_condition_expression(value, arg_map=arg_map)
            )
            ...
            io = self.get_node("field", block).text
            return "params['events']['gpio'].values['cgpio']['digital'][{}] if 'gpio' in params['events'] else 1".format(
                io
            )
        elif block.attrib["type"] == "gpio_get_co":
            io = self.get_node("field", block).text
            return "params['events']['gpio'].values['cgpio']['digital_o'][{}] if 'gpio' in params['events'] else 0".format(
                io
            )
        elif block.attrib["type"] == "gpio_get_ai":
            io = self.get_node("field", block).text
            return "params['events']['gpio'].values['cgpio']['analog'][{}] if 'gpio' in params['events'] else 0".format(
                io
            )
        elif block.attrib["type"] == "gpio_get_ao":
            io = self.get_node("field", block).text
            return "params['events']['gpio'].values['cgpio']['analog_o'][{}] if 'gpio' in params['events'] else 0".format(
                io
            )
            ...
            fields = self.get_nodes("field", root=block)
            timeout = float(fields[0].text)
            return "arm.arm.check_bio_gripper_is_catch(timeout={}) == True".format(
                timeout
            )
    ...
    def _handle_set_line_track(self, block, prefix="", arg_map=None):
        ...
        self._append_to_file(
            "{}    code = arm.set_linear_track_pos({}, speed={}, wait={}, auto_enable=True)".format(
                prefix, pos, speed, wait
            )
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('set_linear_track_pos, code={{}}'.format(code))".format(
                prefix
            )
        )
    ...
    def _handle_set_line_track_origin(self, block, prefix="", arg_map=None):
        if self._show_comment:
            self._append_to_file(
                "{}# set_line_track_origin(wait=True, auto_enable=True)".format(prefix)
            )
        self._append_to_file(
            "{}if arm.error_code == 0 and not params['quit']:".format(prefix)
        )
        self._append_to_file(
            "{}    code = arm.set_linear_track_back_origin(wait=True, auto_enable=True)".format(
                prefix
            )
        )
        self._append_to_file(
            "{}    code = arm.set_linear_track_enable(True)".format(prefix)
        )
        self._append_to_file("{}    if code != 0:".format(prefix))
        self._append_to_file("{}        params['quit'] = True".format(prefix))
        self._append_to_file(
            "{}        pprint('line_track_back_origin, code={{}}'.format(code))".format(
                prefix
            )
        )
...
if __name__ == "__main__":
    blockly = BlocklyTool(
        "C:\\Users\\ufactory\\.UFACTORY\projects\\test\\xarm6\\app\\myapp\local_test_1\\app.xml"
    )
