...
class _BlocklyBase(_BlocklyNode):
    ...
    def _get_field_value(self, block):
        ...
        if field is not None:
            ...
            return self._get_nodes(
                "field", root=self._get_node("value", root=block), descendant=True
            )[0].text
    ...
    def _get_condition_expression(self, value_block, arg_map=None):
        ...
        if block.attrib["type"] == "logic_boolean":
            ...
            return "not ({})".format(
                self._get_condition_expression(value, arg_map=arg_map)
            )
            ...
            io = self._get_node("field", block).text
            return "1 if self._cgpio_state is None else self._cgpio_state[3] >> {} & 0x0001".format(
                io
            )
        elif block.attrib["type"] == "gpio_get_co":
            io = self._get_node("field", block).text
            return "0 if self._cgpio_state is None else self._cgpio_state[5] >> {} & 0x0001".format(
                io
            )
        elif block.attrib["type"] == "gpio_get_ai":
            io = self._get_node("field", block).text
            return (
                "0 if self._cgpio_state is None else self._cgpio_state[6 + {}]".format(
                    io
                )
            )
            ...
            io = self._get_node("field", block).text
            return (
                "0 if self._cgpio_state is None else self._cgpio_state[8 + {}]".format(
                    io
                )
            )
            ...
            return "self._arm.arm.check_air_pump_state({}, timeout={})".format(
                state, timeout
            )
        elif block.attrib["type"] == "check_bio_gripper_is_catch":
            fields = self._get_nodes("field", root=block)
            timeout = float(fields[0].text)
            return "self._arm.arm.check_bio_gripper_is_catch(timeout={})".format(
                timeout
            )
            ...
            for field in fields[:-1]:
                values.append(int(field.text))
            timeout = float(fields[-1].text)
            return "self._arm.arm.get_tgpio_li_state({}, timeout={})".format(
                values, timeout
            )
