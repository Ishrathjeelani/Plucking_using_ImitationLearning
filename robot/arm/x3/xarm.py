...
class XArm(Gripper, Servo, Record, RobotIQ, BaseBoard, Track, FtSensor, ModbusTcp):
    ...
    def __init__(
        self, port=None, is_radian=False, do_not_open=False, instance=None, **kwargs
    ):
    ...
    def _is_out_of_tcp_range(self, value, i):
        if (
            not self._check_tcp_limit
            or self._stream_type != "socket"
            or not self._enable_report
        ):
            return False
        ...
        if 2 < i < len(tcp_range):  # only limit rotate
            ...
            if limit[0] == limit[1]:
                return False
            if value < limit[0] - math.radians(0.1) or value > limit[1] + math.radians(
                0.1
            ):
                self.log_api_info(
                    "API -> set_position -> out_of_tcp_range -> code={}, i={} value={}".format(
                        APIState.OUT_OF_RANGE, i, value
                    ),
                    code=APIState.OUT_OF_RANGE,
                )
                return True
        return False
    ...
    def _is_out_of_joint_range(self, angle, i):
        ...
        if i < len(joint_limit):
            ...
            if angle < angle_range[0] - math.radians(0.1) or angle > angle_range[
                1
            ] + math.radians(0.1):
                self.log_api_info(
                    "API -> set_servo_angle -> out_of_joint_range -> code={}, i={} value={}".format(
                        APIState.OUT_OF_RANGE, i, angle
                    ),
                    code=APIState.OUT_OF_RANGE,
                )
                return True
    ...
    def __get_tcp_motion_params(self, speed=None, mvacc=None, mvtime=None, **kwargs):
        speed = (
            speed if speed is not None else kwargs.get("mvvelo", self._last_tcp_speed)
        )
        ...
        spd = (
            self._last_tcp_speed
            if speed is None
            else min(max(float(speed), self._min_tcp_speed), 1000)
        )
        acc = (
            self._last_tcp_acc
            if mvacc is None
            else min(max(float(mvacc), self._min_tcp_acc), 50000)
        )
        mvt = self._mvtime if mvtime is None else mvtime
        return spd, acc, mvt

    def __get_joint_motion_params(
        self, speed=None, mvacc=None, mvtime=None, is_radian=None, **kwargs
    ):
    ...
    def _set_position_absolute(
        ...
        **kwargs
        ...
        if self.version_is_ge(1, 11, 100) or kwargs.get("debug", False):
            ret = self.arm_cmd.move_line_common(
                tcp_pos,
                spd,
                acc,
                mvt,
                ...
                coord=0,
                is_axis_angle=False,
                only_check_type=only_check_type,
                motion_type=motion_type,
                feedback_key=feedback_key,
            )
        else:
            if radius >= 0:
                ret = self.arm_cmd.move_lineb(
                    tcp_pos,
                    spd,
                    acc,
                    mvt,
                ...
                ret = self.arm_cmd.move_line(
                    tcp_pos, spd, acc, mvt, only_check_type, motion_type=motion_type
                )
        trans_id = self._get_feedback_transid(feedback_key, studio_wait)
        ret[0] = self._check_code(ret[0], is_move_cmd=True)
        self.log_api_info(
            "API -> set_position -> code={}, pos={}, radius={}, velo={}, acc={}".format(
                ret[0], tcp_pos, radius, spd, acc
            ),
            code=ret[0],
        )
    ...
    def _set_position_relative(
        ...
        **kwargs
        ...
        if self.version_is_ge(1, 8, 100):
            ...
            radius = radius if radius is not None else -1
            feedback_key, studio_wait = self._gen_feedback_key(wait, **kwargs)
            ret = self.arm_cmd.move_relative(
                tcp_pos,
                spd,
                acc,
                mvt,
            ...
            ret[0] = self._check_code(ret[0], is_move_cmd=True)
            self.log_api_info(
                "API -> set_relative_position -> code={}, pos={}, radius={}, velo={}, acc={}".format(
                    ret[0], tcp_pos, radius, spd, acc
                ),
                code=ret[0],
            )
            ...
            tcp_pos = [
                (
                    ...
                    if x is None
                    else (self._last_position[0] + float(x))
                ),
                (
                    self._last_position[1]
                    if y is None
                    else (self._last_position[1] + float(y))
                ),
                (
                    self._last_position[2]
                    if z is None
                    else (self._last_position[2] + float(z))
                ),
                (
                    self._last_position[3]
                    if roll is None
                    else (self._last_position[3] + to_radian(roll, is_radian))
                ),
                (
                    self._last_position[4]
                    if pitch is None
                    else (self._last_position[4] + to_radian(pitch, is_radian))
                ),
                (
                    self._last_position[5]
                    if yaw is None
                    else (self._last_position[5] + to_radian(yaw, is_radian))
                ),
            ]
    ...
    def set_position(
        ...
        **kwargs
        ...
        if code != 0:
            return code
        if relative:
            return self._set_position_relative(
                x=x,
                y=y,
                z=z,
                roll=roll,
    ...
    def set_tool_position(
        ...
        **kwargs
        ...
        if self.version_is_ge(1, 11, 100) or kwargs.get("debug", False):
            ...
            ret = self.arm_cmd.move_line_tool(
                tcp_pos, spd, acc, mvt, only_check_type, motion_type=motion_type
            )
        trans_id = self._get_feedback_transid(feedback_key, studio_wait)
        ret[0] = self._check_code(ret[0], is_move_cmd=True)
        self.log_api_info(
            "API -> set_tool_position -> code={}, pos={}, velo={}, acc={}".format(
    ...
    def set_position_aa(
        ...
        **kwargs
        ...
        if self.version_is_ge(1, 11, 100) or kwargs.get("debug", False):
            if not is_tool_coord and relative:
                ...
                ret = self.arm_cmd.move_line_common(
                    ...
                    mvt,
                    radius,
                    coord=1 if is_tool_coord else 0,
                    is_axis_angle=True,
                    only_check_type=only_check_type,
                    motion_type=motion_type,
                    feedback_key=feedback_key,
        ...
        ret[0] = self._check_code(ret[0], is_move_cmd=True)
        self.log_api_info(
            "API -> set_position_aa -> code={}, pos={}, velo={}, acc={}".format(
                ret[0], tcp_pos, spd, acc
            ),
            code=ret[0],
        )
    ...
    def set_servo_cartesian_aa(
        ...
        **kwargs
        ...
        assert len(mvpose) >= 6
        tcp_pos = [to_radian(mvpose[i], is_radian or i <= 2) for i in range(6)]
        spd, acc, mvt = self.__get_tcp_motion_params(
            speed, mvacc, self._mvtime, **kwargs
        )

        tool_coord = kwargs.get("tool_coord", int(is_tool_coord))

        self._has_motion_cmd = True
        ret = self.arm_cmd.move_servo_cart_aa(
            mvpose=tcp_pos,
            mvvelo=spd,
            mvacc=acc,
        ...
        self.log_api_info(
            "API -> set_servo_cartesian_aa -> code={}, pose={}, velo={}, acc={}".format(
                ret[0], tcp_pos, spd, acc
            ),
            code=ret[0],
        )
    ...
    def _set_servo_angle_absolute(
        ...
        **kwargs
        ...
        spd, acc, mvt = self.__get_joint_motion_params(
            speed, mvacc, mvtime, is_radian=is_radian, **kwargs
        )
        self._has_motion_cmd = True
        feedback_key, studio_wait = self._gen_feedback_key(wait, **kwargs)
        if self.version_is_ge(1, 5, 20) and radius is not None and radius >= 0:
            ret = self.arm_cmd.move_jointb(
                joints, spd, acc, radius, only_check_type, feedback_key=feedback_key
            )
        else:
            ret = self.arm_cmd.move_joint(
                joints, spd, acc, mvt, only_check_type, feedback_key=feedback_key
            )
        trans_id = self._get_feedback_transid(feedback_key, studio_wait)
        ret[0] = self._check_code(ret[0], is_move_cmd=True)
        self.log_api_info(
            "API -> set_servo_angle -> code={}, angles={}, velo={}, acc={}, radius={}".format(
    ...
    def _set_servo_angle_relative(
        ...
        **kwargs
        ...
        if self.version_is_ge(1, 8, 100):
            ...
            ret[0] = self._check_code(ret[0], is_move_cmd=True)
            self.log_api_info(
                "API -> set_relative_servo_angle -> code={}, angles={}, velo={}, acc={}, radius={}".format(
                    ret[0], joints, spd, acc, radius
                ),
                code=ret[0],
            )
    ...
    def set_servo_angle(
        ...
        **kwargs
        ...
        assert (
            ...
            1 <= servo_id <= 7 and angle is not None and not isinstance(angle, Iterable)
        ...
        if code != 0:
            return code
        if relative:
            return self._set_servo_angle_relative(
                angles,
                speed=speed,
                mvacc=mvacc,
                mvtime=mvtime,
    ...
    def set_servo_angle_j(
        ...
        ret[0] = self._check_code(ret[0], is_move_cmd=True, mode=1)
        self.log_api_info(
            "API -> set_servo_angle_j -> code={}, angles={}, velo={}, acc={}".format(
                ret[0], angs, spd, acc
            ),
            code=ret[0],
        )
    ...
    def set_servo_cartesian(
        ...
        **kwargs
        ...
        ret[0] = self._check_code(ret[0], is_move_cmd=True, mode=1)
        self.log_api_info(
            "API -> set_servo_cartisian -> code={}, pose={}, velo={}, acc={}, is_tool_coord={}".format(
    ...
    def move_circle(
        ...
        **kwargs
        ...
        feedback_key, studio_wait = self._gen_feedback_key(wait, **kwargs)
        if self.version_is_ge(1, 11, 100) or kwargs.get("debug", False):
            ret = self.arm_cmd.move_circle_common(
                pose_1,
                pose_2,
                spd,
                acc,
            ...
            ret = self.arm_cmd.move_circle(
                pose_1, pose_2, spd, acc, mvt, percent, only_check_type
            )
        trans_id = self._get_feedback_transid(feedback_key, studio_wait)
        ret[0] = self._check_code(ret[0], is_move_cmd=True)
        self.log_api_info(
            "API -> move_circle -> code={}, pos1={}, pos2={}, percent={}%, velo={}, acc={}".format(
                ret[0], pose_1, pose_2, percent, spd, acc
            ),
            code=ret[0],
        )
        self._is_set_move = True
    ...
    def move_gohome(
        ...
        **kwargs
        ...
        self._has_motion_cmd = True
        feedback_key, studio_wait = self._gen_feedback_key(wait, **kwargs)
        ret = self.arm_cmd.move_gohome(
            spd, acc, mvt, only_check_type, feedback_key=feedback_key
        )
        trans_id = self._get_feedback_transid(feedback_key, studio_wait)
        ret[0] = self._check_code(ret[0], is_move_cmd=True)
        self.log_api_info(
            "API -> move_gohome -> code={}, velo={}, acc={}".format(ret[0], spd, acc),
            code=ret[0],
        )
        self._is_set_move = True
        self._only_check_result = 0
    ...
    def move_arc_lines(
        ...
        def _move():
            if automatic_calibration:
                ret = self.set_servo_angle(
                    angle=angles, is_radian=True, speed=0.8726646259971648, wait=False
                )
                if ret < 0:
                    return -1
    ...
    def set_servo_detach(self, servo_id=None):
        ...
        """
        assert (
            isinstance(servo_id, int) and 1 <= servo_id <= 8
        ), "The value of parameter servo_id can only be 1-8."
        ret = self.arm_cmd.set_brake(servo_id, 1)
        self.log_api_info(
            "API -> set_servo_detach -> code={}".format(ret[0]), code=ret[0]
        )
        self._sync()
        return ret[0]
    ...
    def system_control(self, value=1):
        ...
        self.log_api_info(
            "API -> system_control({}) -> code={}".format(value, ret[0]), code=ret[0]
        )
        return ret[0]
    ...
    def set_reduced_mode(self, on_off):
        ...
        self.log_api_info(
            "API -> set_reduced_mode -> code={}".format(ret[0]), code=ret[0]
        )
        return ret[0]
    ...
    def set_reduced_max_tcp_speed(self, speed):
        ...
        self.log_api_info(
            "API -> set_reduced_linespeed -> code={}, speed={}".format(ret[0], speed),
            code=ret[0],
        )
        return ret[0]
    ...
    def get_reduced_states(self, is_radian=None):
        ...
        ret = self.arm_cmd.get_reduced_states(
            79 if self.version_is_ge(1, 2, 11) else 21
        )
        ret[0] = self._check_code(ret[0])
        if ret[0] == 0:
            if not is_radian:
    ...
    def set_reduced_tcp_boundary(self, boundary):
        ...
        limits = [0] * 6
        limits[0:2] = (
            boundary[0:2] if boundary[0] >= boundary[1] else boundary[0:2][::-1]
        )
        limits[2:4] = (
            boundary[2:4] if boundary[2] >= boundary[3] else boundary[2:4][::-1]
        )
        limits[4:6] = (
            boundary[4:6] if boundary[4] >= boundary[5] else boundary[4:6][::-1]
        )
        ret = self.arm_cmd.set_xyz_limits(limits)
        self.log_api_info(
            "API -> set_reduced_tcp_boundary -> code={}, boundary={}".format(
                ret[0], limits
            ),
            code=ret[0],
        )
    ...
    def set_reduced_joint_range(self, joint_range, is_radian=None):
        ...
        for i in range(7):
            if i < self.axis:
                limits[i * 2 : i * 2 + 2] = (
                    joint_range[i * 2 : i * 2 + 2]
                    if joint_range[i * 2] <= joint_range[i * 2 + 1]
                    else joint_range[i * 2 : i * 2 + 2][::-1]
                )
        if not is_radian:
        ...
        for i in range(self.axis):
            joint_limit = XCONF.Robot.JOINT_LIMITS.get(self.axis).get(
                self.device_type, []
            )
            if i < len(joint_limit):
                angle_range = joint_limit[i]
        ...
        self.log_api_info(
            "API -> set_reduced_joint_range -> code={}, boundary={}".format(
                ret[0], limits
            ),
            code=ret[0],
        )
    ...
    def set_fense_mode(self, on_off):
        ...
        self.log_api_info(
            "API -> set_fense_mode -> code={}, on={}".format(ret[0], on_off),
            code=ret[0],
        )
        return ret
    ...
    def set_collision_rebound(self, on_off):
        ...
        self.log_api_info(
            "API -> set_collision_rebound -> code={}, on={}".format(ret[0], on_off),
            code=ret[0],
        )
        return ret
    ...
    def set_world_offset(self, offset, is_radian=None, wait=True):
        ...
        self.log_api_info(
            "API -> set_world_offset -> code={}, offset={}".format(
                ret[0], world_offset
            ),
            code=ret[0],
        )
    ...
    def set_safe_level(self, level=4):
        ...
        self.log_api_info(
            "API -> set_safe_level -> code={}, level={}".format(ret[0], level),
            code=ret[0],
        )
        return ret[0]
    ...
    def set_pause_time(self, sltime, wait=False):
        ...
        self.log_api_info(
            "API -> set_pause_time -> code={}, sltime={}".format(ret[0], sltime),
            code=ret[0],
        )
        return ret[0]
    ...
    def set_tcp_offset(self, offset, is_radian=None, wait=True, **kwargs):
        ...
        self.log_api_info(
            "API -> set_tcp_offset -> code={}, offset={}".format(ret[0], tcp_offset),
            code=ret[0],
        )
        return ret[0]
    ...
    def set_tcp_jerk(self, jerk):
        ...
        self.log_api_info(
            "API -> set_tcp_jerk -> code={}, jerk={}".format(ret[0], jerk), code=ret[0]
        )
        return ret[0]
    ...
    def set_tcp_maxacc(self, acc):
        ...
        self.log_api_info(
            "API -> set_tcp_maxacc -> code={}, maxacc={}".format(ret[0], acc),
            code=ret[0],
        )
        return ret[0]
    ...
    def set_joint_jerk(self, jerk, is_radian=None):
        ...
        self.log_api_info(
            "API -> set_joint_jerk -> code={}, jerk={}".format(ret[0], jerk),
            code=ret[0],
        )
        return ret[0]
    ...
    def set_joint_maxacc(self, maxacc, is_radian=None):
        ...
        self.log_api_info(
            "API -> set_joint_maxacc -> code={}, maxacc={}".format(ret[0], maxacc),
            code=ret[0],
        )
        return ret[0]
    ...
    def set_collision_sensitivity(self, value, wait=True):
        ...
        self.set_state(0)
        self.log_api_info(
            "API -> set_collision_sensitivity -> code={}, sensitivity={}".format(
                ret[0], value
            ),
            code=ret[0],
        )
    ...
    def set_teach_sensitivity(self, value, wait=True):
        ...
        self.log_api_info(
            "API -> set_teach_sensitivity -> code={}, sensitivity={}".format(
                ret[0], value
            ),
            code=ret[0],
        )
    ...
    def set_gravity_direction(self, direction, wait=True):
        ...
        self.log_api_info(
            "API -> set_gravity_direction -> code={}, direction={}".format(
                ret[0], direction
            ),
            code=ret[0],
        )
    ...
    def set_mount_direction(self, base_tilt_deg, rotation_deg, is_radian=None):
        ...
        for i in range(3):
            ...
            g_new[i] = (
                Rot[i * 3 + 0] * G_normal[0]
                + Rot[i * 3 + 1] * G_normal[1]
                + Rot[i * 3 + 2] * G_normal[2]
            )

        ret = self.arm_cmd.set_gravity_dir(g_new)
        self.log_api_info(
            "API -> set_mount_direction -> code={}, tilt={}, rotation={}, direction={}".format(
                ret[0], base_tilt_deg, rotation_deg, g_new
            ),
            code=ret[0],
        )
        return ret[0]
    ...
    def get_inverse_kinematics(self, pose, input_is_radian=None, return_is_radian=None):
        input_is_radian = (
            self._default_is_radian if input_is_radian is None else input_is_radian
        )
        return_is_radian = (
            self._default_is_radian if return_is_radian is None else return_is_radian
        )
        assert len(pose) >= 6
    ...
    def get_forward_kinematics(
        self, angles, input_is_radian=None, return_is_radian=None
    ):
        input_is_radian = (
            self._default_is_radian if input_is_radian is None else input_is_radian
        )
        ...
        if ret[0] == 0:
            ...
            if not return_is_radian:
                pose = [
                    pose[i] if i < 3 else math.degrees(pose[i])
    ...
    def is_tcp_limit(self, pose, is_radian=None):
        ...
        assert len(pose) >= 6
        tcp_pose = [
            to_radian(pose[i], is_radian or i <= 2, self._last_position[i])
            for i in range(6)
        ]
        ret = self.arm_cmd.is_tcp_limit(tcp_pose)
        self.log_api_info(
            "API -> is_tcp_limit -> code={}, limit={}".format(ret[0], ret[1]),
            code=ret[0],
        )
        ret[0] = self._check_code(ret[0])
        if ret[0] == 0:
    ...
    def is_joint_limit(self, joint, is_radian=None):
        ...
        self.log_api_info(
            "API -> is_joint_limit -> code={}, limit={}".format(ret[0], ret[1]),
            code=ret[0],
        )
        ret[0] = self._check_code(ret[0])
        if ret[0] == 0:
    ...
    def _handle_gcode(self, command):
        def __handle_gcode_g(num):
            if (
                num == 1
            ):  # G1 move_line, ex: G1 X{} Y{} Z{} A{roll} B{pitch} C{yaw} F{speed} Q{acc} T{}
                ...
                ret = self.set_position(
                    *mvpose, radius=-1, speed=mvvelo, mvacc=mvacc, mvtime=mvtime
                )
            elif (
                num == 2
            ):  # G2 move_circle, ex: G2 X{} Y{} Z{} A{} B{} C{} I{} J{} K{} L{} M{} N{} F{speed} Q{acc} T{}
                mvvelo = gcode_p.get_mvvelo(command)
                mvacc = gcode_p.get_mvacc(command)
                mvtime = gcode_p.get_mvtime(command)
                ...
                num == 7
            ):  # G7 move_joint, ex: G7 I{} J{} K{} L{} M{} N{} O{} F{} Q{} T{}
                mvvelo = gcode_p.get_mvvelo(command)
                mvacc = gcode_p.get_mvacc(command)
                mvtime = gcode_p.get_mvtime(command)
                mvjoint = gcode_p.get_joints(command)
                ret = self.set_servo_angle(
                    angle=mvjoint, speed=mvvelo, mvacc=mvacc, mvtime=mvtime
                )
                ...
                num == 9
            ):  # G9 move_arc_line, ex: G9 X{} Y{} Z{} A{roll} B{pitch} C{yaw} R{radius} F{speed} Q{acc} T{}
                mvvelo = gcode_p.get_mvvelo(command)
                mvacc = gcode_p.get_mvacc(command)
                mvtime = gcode_p.get_mvtime(command)
                mvpose = gcode_p.get_poses(command)
                ...
                ret = self.set_position(
                    *mvpose, speed=mvvelo, mvacc=mvacc, mvtime=mvtime, radius=mvradii
                )
            elif (
                num == 11
            ):  # G11 set_servo_angle_j, ex: G11 I{} J{} K{} L{} M{} N{} O{} F{} Q{} T{}
                mvvelo = gcode_p.get_mvvelo(command)
                mvacc = gcode_p.get_mvacc(command)
                mvtime = gcode_p.get_mvtime(command)
                mvjoint = gcode_p.get_joints(command, default=0)
                ret = self.set_servo_angle_j(
                    mvjoint, speed=mvvelo, mvacc=mvacc, mvtime=mvtime
                )
        ...
        def __handle_gcode_h(num):
            if num == 1:  # H1 get_version, ex: H1
                ...
                num == 35
            ):  # H35 set_tcp_offset, ex: H35 X{x} Y{y} Z{z} A{roll} B{pitch} C{yaw}
                pose = gcode_p.get_poses(command)
                ret = self.set_tcp_offset(pose)
            elif (
                num == 36
            ):  # H36 set_tcp_load, ex: H36 I{weight} J{center_x} K{center_y} L{center_z}
                ...
                ret = self.get_inverse_kinematics(
                ...
                ret = self.get_forward_kinematics(
                    joint, input_is_radian=False, return_is_radian=False
                )
                ...
                num == 46
            ):  # H46 is_tcp_limit, ex: H46 X{} Y{} Z{} A{roll} B{pitch} C{yaw}
                pose = gcode_p.get_poses(command)
                ret = self.is_tcp_limit(pose, is_radian=False)
            elif (
                num == 51
            ):  # H51 set_gravity_direction, ex: H51 X{} Y{} Z{} A{roll} B{pitch} C{yaw}
                pose = gcode_p.get_poses(command, default=0)
        ...
        def __handle_gcode_c(num):
            if num == 131:  # C131 get_cgpio_digital, ex: C131
                ...
                num == 135
            ):  # C135 set_cgpio_analog(0, v), ex: C135 I{ionum=0} V{value}
                ionum = gcode_p.get_id_num(command, default=0)
                value = gcode_p.get_float_value(command)
                ret = self.set_cgpio_analog(ionum, value)
            elif (
                num == 136
            ):  # C136 set_cgpio_analog(1, v), ex: C136 I{ionum=1} V{value}
                ionum = gcode_p.get_id_num(command, default=1)
                value = gcode_p.get_float_value(command)
                ret = self.set_cgpio_analog(ionum, value)
            elif (
                num == 137
            ):  # C137 set_cgpio_digital_input_function, ex: C137 I{ionum} V{fun}
                ionum = gcode_p.get_id_num(command, default=0)
                value = gcode_p.get_int_value(command)
                ret = self.set_cgpio_digital_input_function(ionum, value)
            elif (
                num == 138
            ):  # C138 set_cgpio_digital_output_function, ex: C138 I{ionum} V{fun}
                ionum = gcode_p.get_id_num(command, default=0)
                value = gcode_p.get_int_value(command)
    ...
    def run_blockly_app(self, path, **kwargs):
        ...
        try:
            ...
            succeed = blockly_tool.to_python(
                arm=self._api_instance, is_exec=True, **kwargs
            )
            if succeed:
                times = kwargs.get("times", 1)
                highlight_callback = kwargs.get("highlight_callback", None)
                blockly_print = kwargs.get("blockly_print", print)
                connect_changed_callbacks = self._report_callbacks[
                    self.REPORT_CONNECT_CHANGED_ID
                ].copy()
                state_changed_callbacks = self._report_callbacks[
                    self.REPORT_STATE_CHANGED_ID
                ].copy()
                error_warn_changed_callbacks = self._report_callbacks[
                    self.REPORT_ERROR_WARN_CHANGED_ID
                ].copy()
                count_changed_callbacks = self._report_callbacks[
                    self.REPORT_COUNT_CHANGED_ID
                ].copy()
                ...
                self._report_callbacks[self.REPORT_CONNECT_CHANGED_ID] = (
                    connect_changed_callbacks
                )
                self._report_callbacks[self.REPORT_STATE_CHANGED_ID] = (
                    state_changed_callbacks
                )
                self._report_callbacks[self.REPORT_ERROR_WARN_CHANGED_ID] = (
                    error_warn_changed_callbacks
                )
                self._report_callbacks[self.REPORT_COUNT_CHANGED_ID] = (
                    count_changed_callbacks
                )
                return code
            else:
                logger.error(
                    "The conversion is incomplete and some blocks are not yet supported."
                )
    ...
    def reload_dynamics(self):
        ...
        ret[0] = self._check_code(ret[0])
        self.log_api_info(
            "API -> reload_dynamics -> code={}".format(ret[0]), code=ret[0]
        )
        return ret[0]
    ...
    def set_counter_reset(self):
        ...
        ret[0] = self._check_code(ret[0])
        self.log_api_info(
            "API -> set_counter_reset -> code={}".format(ret[0]), code=ret[0]
        )
        return ret[0]
    ...
    def set_counter_increase(self, val=1):
        ...
        ret[0] = self._check_code(ret[0])
        self.log_api_info(
            "API -> set_counter_increase -> code={}".format(ret[0]), code=ret[0]
        )
        return ret[0]
    ...
    def set_report_tau_or_i(self, tau_or_i=0):
        ...
        ret[0] = self._check_code(ret[0])
        self.log_api_info(
            "API -> set_report_tau_or_i({}) -> code={}".format(tau_or_i, ret[0]),
            code=ret[0],
        )
        return ret[0]
    ...
    def set_self_collision_detection(self, on_off):
        ...
        ret[0] = self._check_code(ret[0])
        self.log_api_info(
            "API -> set_self_collision_detection({}) -> code={}".format(on_off, ret[0]),
            code=ret[0],
        )
        return ret[0]
    ...
    def set_collision_tool_model(self, tool_type, *args, **kwargs):
        if tool_type == XCONF.CollisionToolType.BOX:
            assert (
                ("z" in kwargs or len(args) >= 3)
                and ("y" in kwargs or len(args) >= 2)
                and ("x" in kwargs or len(args) >= 1)
            ), "params error, must specify x,y,z parameter"
            ...
            assert ("radius" in kwargs or len(args) >= 2) and (
                "height" in kwargs or len(args) >= 1
            ), "params error, must specify radius,height parameter"
            radius = kwargs.get("radius") if "radius" in kwargs else args[0]
            height = kwargs.get("height") if "height" in kwargs else args[1]
            params = [radius, height]
        else:
            params = (
                [] if tool_type < XCONF.CollisionToolType.USE_PRIMITIVES else list(args)
            )
        ret = self.arm_cmd.set_collision_tool_model(tool_type, params)
        ret[0] = self._check_code(ret[0])
        self.log_api_info(
            "API -> set_collision_tool_model({}, {}) -> code={}".format(
                tool_type, params, ret[0]
            ),
            code=ret[0],
        )
        return ret[0]
    ...
    def get_firmware_config(self):
        ...
        code = (
            cgpio_code
            if reduced_code == 0 and tau_code == 0
            else reduced_code if cgpio_code == 0 and tau_code == 0 else tau_code
        )
        return code, {
    ...
    def set_firmware_config(self, config):
        ...
        if (
            "JOINT_MAXACC" in config
            and config["JOINT_MAXACC"] != old_config["JOINT_MAXACC"]
        ):
        ...
        if (
            "WORLD_OFFSET" in config
            and config["WORLD_OFFSET"] != old_config["WORLD_OFFSET"]
        ):
            self.set_world_offset(config["WORLD_OFFSET"])
        if (
            "REPORT_TAU_OR_I" in config
            and config["REPORT_TAU_OR_I"] != old_config["REPORT_TAU_OR_I"]
        ):
            self.set_report_tau_or_i(config["REPORT_TAU_OR_I"])
        if (
            "GPIO_RESET_CONFIG" in config
            and config["GPIO_RESET_CONFIG"] != old_config["GPIO_RESET_CONFIG"]
        ):
        ...
        if (
            "CGPIO_INPUT_FUNC_CONFIG" in config
            and config["CGPIO_INPUT_FUNC_CONFIG"]
            != old_config["CGPIO_INPUT_FUNC_CONFIG"]
        ):
            for i in range(len(config["CGPIO_INPUT_FUNC_CONFIG"])):
                if (
                    config["CGPIO_INPUT_FUNC_CONFIG"][i]
                    != old_config["CGPIO_INPUT_FUNC_CONFIG"][i]
                ):
                    self.set_cgpio_digital_input_function(
                        i, config["CGPIO_INPUT_FUNC_CONFIG"][i]
                    )
        if (
            "CGPIO_OUTPUT_FUNC_CONFIG" in config
            and config["CGPIO_OUTPUT_FUNC_CONFIG"]
            != old_config["CGPIO_OUTPUT_FUNC_CONFIG"]
        ):
            for i in range(len(config["CGPIO_OUTPUT_FUNC_CONFIG"])):
                if (
                    config["CGPIO_OUTPUT_FUNC_CONFIG"][i]
                    != old_config["CGPIO_OUTPUT_FUNC_CONFIG"][i]
                ):
                    self.set_cgpio_digital_output_function(
                        i, config["CGPIO_OUTPUT_FUNC_CONFIG"][i]
                    )

        if (
            "COLL_PARAMS" in config
            and config["COLL_PARAMS"] != old_config["COLL_PARAMS"]
        ):
            if config["COLL_PARAMS"][0] != old_config["COLL_PARAMS"][0]:
                self.set_self_collision_detection(config["COLL_PARAMS"][0])
            if (
                config["COLL_PARAMS"][1] != old_config["COLL_PARAMS"][1]
                or config["COLL_PARAMS"][2] != old_config["COLL_PARAMS"][2]
            ):
                self.set_collision_tool_model(
                    config["COLL_PARAMS"][1], *config["COLL_PARAMS"][2]
                )
    ...
    def vc_set_joint_velocity(
        self, speeds, is_radian=None, is_sync=True, check_mode=True, duration=-1
    ):
        ...
        ret = self.arm_cmd.vc_set_jointv(
            jnt_v, 1 if is_sync else 0, duration if self.version_is_ge(1, 8, 0) else -1
        )
        ret[0] = self._check_code(ret[0], is_move_cmd=True, mode=4)
        self.log_api_info(
            "API -> vc_set_joint_velocity -> code={}, speeds={}, is_sync={}".format(
                ret[0], jnt_v, is_sync
            ),
            code=ret[0],
        )
    ...
    @xarm_is_connected(_type="set")
    def vc_set_cartesian_velocity(
        self, speeds, is_radian=None, is_tool_coord=False, check_mode=True, duration=-1
    ):
        ...
        ret = self.arm_cmd.vc_set_linev(
            line_v,
            1 if is_tool_coord else 0,
            duration if self.version_is_ge(1, 8, 0) else -1,
        )
        ret[0] = self._check_code(ret[0], is_move_cmd=True, mode=5)
        self.log_api_info(
            "API -> vc_set_cartesian_velocity -> code={}, speeds={}, is_tool_coord={}".format(
    ...
    def calibrate_tcp_coordinate_offset(self, four_points, is_radian=None):
        assert (
            len(four_points) >= 4
        ), "The parameter four_points must contain 4 TCP points"
        is_radian = self._default_is_radian if is_radian is None else is_radian
        points = []
        for i in range(4):
            assert (
                len(four_points[i]) >= 6
            ), "Each TCP point in the parameter four_points must contain x/y/z/roll/pitch/yaw"
            points.append(
                [
                    (
                        four_points[i][j]
                        if j <= 2
                        else to_radian(four_points[i][j], is_radian)
                    )
                    for j in range(6)
                ]
            )
    ...
    def calibrate_tcp_orientation_offset(
        ...
        ret[0] = self._check_code(ret[0])
        return ret[0], [
            ret[i + 1] if return_is_radian else math.degrees(ret[i + 1])
            for i in range(3)
        ]

    @xarm_is_connected(_type="get")
    def calibrate_user_orientation_offset(
        self,
        three_points,
        mode=0,
        trust_ind=0,
        input_is_radian=None,
        return_is_radian=None,
    ):
        assert (
            len(three_points) >= 3
        ), "The parameter three_points must contain 3 TCP points"
        input_is_radian = (
            self._default_is_radian if input_is_radian is None else input_is_radian
        )
        return_is_radian = (
        ...
        for i in range(3):
            assert (
                len(three_points[i]) >= 6
            ), "Each TCP point in the parameter three_points must contain x/y/z/roll/pitch/yaw"
            points.append(
                [
                    (
                        three_points[i][j]
                        if j <= 2
                        else to_radian(three_points[i][j], input_is_radian)
                    )
                    for j in range(6)
                ]
            )
    ...
    def calibrate_user_coordinate_offset(self, rpy_ub, pos_b_uorg, is_radian=None):
        is_radian = self._default_is_radian if is_radian is None else is_radian
        rpy_ub_ = [
            rpy_ub[i] if is_radian else math.radians(rpy_ub[i]) for i in range(3)
        ]
    ...
    def get_tcp_rotation_radius(self, value=6):
        ...
        self.log_api_info(
            "API -> get_tcp_rotation_radius -> code={}".format(ret[0]), code=ret[0]
        )
        ret[0] = self._check_code(ret[0])
    ...
    def set_cartesian_velo_continuous(self, on_off):
        ...
        ret[0] = self._check_code(ret[0])
        self.log_api_info(
            "API -> set_cartesian_velo_continuous({}) -> code={}".format(
                on_off, ret[0]
            ),
            code=ret[0],
        )
    ...
    def set_allow_approx_motion(self, on_off):
        ...
        ret[0] = self._check_code(ret[0])
        self.log_api_info(
            "API -> set_allow_approx_motion({}) -> code={}".format(on_off, ret[0]),
            code=ret[0],
        )
        return ret[0]
    ...
    def get_allow_approx_motion(self):
        ...
        ret[0] = self._check_code(ret[0])
        self.log_api_info(
            "API -> get_allow_approx_motion() -> code={}".format(ret[0]), code=ret[0]
        )
        return ret[0], ret[-1]
    ...
    def iden_joint_friction(self, sn=None):
        if sn is None:
            ...
            if code != 0:
                self.log_api_info(
                    "iden_joint_friction -> get_robot_sn failed, code={}".format(code),
                    code=code,
                )
                return APIState.API_EXCEPTION, -1
        if len(sn) != 14:
            self.log_api_info(
                "iden_joint_friction, sn is not correct, sn={}".format(sn),
                code=APIState.API_EXCEPTION,
            )
            return APIState.API_EXCEPTION, -1
        sn = sn.upper()
        ...
        valid_xarm = (
            not self.is_850
            and not self.is_lite6
            and sn[0] == "X"
            and sn[1] == axis_map.get(self.axis, "")
        )
        if not (valid_850 or valid_lite or valid_xarm):
            self.log_api_info(
                "iden_joint_friction, sn is not correct, axis={}, type={}, sn={}".format(
                    self.axis, self.device_type, sn
                ),
                code=APIState.API_EXCEPTION,
            )
            return APIState.API_EXCEPTION, -1
        ...
        self._keep_heart = True
        self.log_api_info(
            "API -> iden_joint_friction -> code={}".format(ret[0]), code=ret[0]
        )
