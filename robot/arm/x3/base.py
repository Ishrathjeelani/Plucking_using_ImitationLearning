...
from ..core.config.x_code import (
    ControllerWarn,
    ControllerError,
...
class Base(BaseObject, Events):
    def __init__(self, port=None, is_radian=False, do_not_open=False, **kwargs):
        if kwargs.get("init", False):
            ...
            self._default_linear_track_baud = kwargs.get(
                "default_linear_track_baud", 2000000
            )
            ...
            self._rewrite_modbus_baudrate_method = kwargs.get(
                "rewrite_modbus_baudrate_method", True
            )
            ...
            self._last_angles = [
                ...
                0.0,
                0.0,
            ]  # [servo_1(rad), servo_2(rad), servo_3(rad), servo_4(rad), servo_5(rad), servo_6(rad), servo_7(rad)]
            ...
            self._arm_motor_brake_states = [
                ...
                -1,
                -1,
            ]  # [motor-1-brake-state, ..., motor-7-brake, reserved]
            self._arm_motor_enable_states = [
                -1,
                -1,
                -1,
                -1,
                ...
                -1,
                -1,
            ]  # [motor-1-enable-state, ..., motor-7-enable, reserved]
        ...
        try:
            ...
            if self._version and isinstance(self._version, str):
                ...
                if m:
                    ...
                    self._arm_type_is_1300 = (
                        int(xarm_sn[2:6]) >= 1300 if xarm_sn[2:6].isdigit() else False
                    )
                    self._control_box_type_is_1300 = (
                        int(ac_version[2:6]) >= 1300
                        if ac_version[2:6].isdigit()
                        else False
                    )
                else:
            ...
            if is_first:
                ...
                if self.warn_code != 0:
                    self.clean_warn()
                print(
                    "ROBOT_IP: {}, VERSION: v{}, PROTOCOL: {}, DETAIL: {}, TYPE1300: [{:d}, {:d}]".format(
                        self._port,
                        "{}.{}.{}".format(
                            self._major_version_number,
                            self._minor_version_number,
                        ...
                        "V0" if self._is_old_protocol else "V1",
                        self._version,
                        self._control_box_type_is_1300,
                        self._arm_type_is_1300,
                    )
    ...
    def realtime_joint_speeds(self):
        return [
            speed if self._default_is_radian else math.degrees(speed)
            for speed in self._realtime_joint_speeds
        ]
    ...
    def position(self):
        ...
        return [
            (
                math.degrees(self._position[i])
                if 2 < i < 6 and not self._default_is_radian
                else self._position[i]
            )
    ...
    def position_aa(self):
        ...
        return [
            (
                math.degrees(self._pose_aa[i])
                if 2 < i < 6 and not self._default_is_radian
                else self._pose_aa[i]
            )
    ...
    def last_used_position(self):
        return [
            (
                math.degrees(self._last_position[i])
                if 2 < i < 6 and not self._default_is_radian
                else self._last_position[i]
            )
            for i in range(len(self._last_position))
        ]
    ...
    def angles(self):
        ...
        return [
            angle if self._default_is_radian else math.degrees(angle)
    ...
    def joint_jerk(self):
        return (
            ...
            else math.degrees(self._joint_jerk)
        )
    ...
    def last_used_angles(self):
        return [
            angle if self._default_is_radian else math.degrees(angle)
            for angle in self._last_angles
        ]
    ...
    def position_offset(self):
        return [
            (
                math.degrees(self._position_offset[i])
                if 2 < i < 6 and not self._default_is_radian
                else self._position_offset[i]
            )
            for i in range(len(self._position_offset))
        ]
    ...
    def world_offset(self):
        return [
            (
                math.degrees(self._world_offset[i])
                if 2 < i < 6 and not self._default_is_radian
                else self._world_offset[i]
            )
            for i in range(len(self._world_offset))
        ]
    ...
    def has_err_warn(self):
        return (
            self.has_error
            or self._warn_code != 0
            or (self.arm_cmd and self.arm_cmd.has_err_warn)
        )
    ...
    def self_collision_params(self):
        return [
            self._is_collision_detection,
            self._collision_tool_type,
    ...
    def version_is_ge(self, major, minor=0, revision=0):
        if self._version is None:
            self._check_version()
        return (
            self._major_version_number > major
            or (
                self._major_version_number == major
                and self._minor_version_number > minor
            )
            or (
                self._major_version_number == major
                and self._minor_version_number == minor
                and self._revision_version_number >= revision
            )
        )

    def wait_until_not_pause(self):
        if (
            self._check_is_pause
            and self.connected
            and self.state == 3
            and self._enable_report
        ):
    ...
    def _timed_comm_thread(self):
        ...
        while self.connected and self._timed_comm_t_alive:
            ...
            if (
                self.arm_cmd
                and curr_time - last_send_time > 10
                and curr_time - self.arm_cmd.last_comm_time > self._timed_comm_interval
            ):
                try:
                    if cnt == 0:
    ...
    def connect_503(self):
        self._stream_503 = SocketPort(
            self._port,
            XCONF.SocketConf.TCP_CONTROL_PORT + 1,
        ...
        if not self.connected_503:
            return -1
        self.arm_cmd_503 = UxbusCmdTcp(
            self._stream_503, set_feedback_key_tranid=self._set_feedback_key_tranid
        )
    ...
    def connect(self, port=None, baudrate=None, timeout=None, axis=None, arm_type=None):
        ...
        if isinstance(self._port, (str, bytes)):
            if self._port == "localhost" or re.match(
                ...
                self._stream = SocketPort(
                    self._port,
                    XCONF.SocketConf.TCP_CONTROL_PORT,
                    heartbeat=self._enable_heartbeat,
                    buffer_size=XCONF.SocketConf.TCP_CONTROL_BUF_SIZE,
                    forbid_uds=self._forbid_uds,
                    fb_que=self._feedback_que,
                )
                ...
                self._feedback_thread = threading.Thread(
                    target=self._feedback_thread_handle, daemon=True
                )
                self._feedback_thread.start()

                self.arm_cmd = UxbusCmdTcp(
                    self._stream, set_feedback_key_tranid=self._set_feedback_key_tranid
                )
                ...
                try:
                    if self._timed_comm:
                        self._timed_comm_t = threading.Thread(
                            target=self._timed_comm_thread, daemon=True
                        )
                ...
                if self._stream.connected and self._enable_report:
                    self._report_thread = threading.Thread(
                        target=self._report_thread_handle, daemon=True
                    )
                ...
                if SerialPort is None:
                    raise Exception(
                        "serial module is not found, if you want to connect to xArm with serial, please `pip install pyserial==3.4`"
                    )
                ...
                if self._enable_report:
                    self._report_thread = threading.Thread(
                        target=self._auto_get_report_thread, daemon=True
                    )
                    self._report_thread.start()
            ...
            if self._rewrite_modbus_baudrate_method:
                setattr(
                    self.arm_cmd,
                    "set_modbus_baudrate_old",
                    self.arm_cmd.set_modbus_baudrate,
                )
                setattr(
                    self.arm_cmd, "set_modbus_baudrate", self._core_set_modbus_baudrate
                )
        ...
        if self._enable_report:
            ...
            if self._report_type == "real":
                ...
                self._stream_report = SocketPort(
                    ...
                    buffer_size=(
                        XCONF.SocketConf.TCP_REPORT_NORMAL_BUF_SIZE
    ...
    def _report_connect_changed_callback(
        self, main_connected=None, report_connected=None
    ):
        if self.REPORT_CONNECT_CHANGED_ID in self._report_callbacks.keys():
            for callback in self._report_callbacks[self.REPORT_CONNECT_CHANGED_ID]:
                self._run_callback(
                    ...
                    {
                        "connected": (
                            self._stream and self._stream.connected
                            if main_connected is None
                            else main_connected
                        ),
                        "reported": (
                            self._stream_report and self._stream_report.connected
                            if report_connected is None
                            else report_connected
                        ),
                    },
    ...
    def _report_state_changed_callback(self):
        ...
        self.__report_callback(
            self.REPORT_STATE_CHANGED_ID, {"state": self._state}, name="state_changed"
        )

    def _report_mode_changed_callback(self):
        self.__report_callback(
            self.REPORT_MODE_CHANGED_ID, {"mode": self._mode}, name="mode_changed"
        )

    def _report_mtable_mtbrake_changed_callback(self):
        self.__report_callback(
            self.REPORT_MTABLE_MTBRAKE_CHANGED_ID,
            {
    ...
    def _report_temperature_changed_callback(self):
        self.__report_callback(
            self.REPORT_TEMPERATURE_CHANGED_ID,
    ...
    def _report_count_changed_callback(self):
        self.__report_callback(
            self.REPORT_COUNT_CHANGED_ID, {"count": self._count}, name="count_changed"
        )

    def _report_iden_progress_changed_callback(self):
        self.__report_callback(
            self.REPORT_IDEN_PROGRESS_CHANGED_ID,
            {"progress": self._iden_progress},
            name="iden_progress_changed",
        )
            ...
            try:
                ...
                if self._keep_heart:
                    if (
                        ...
                        and self.version_is_ge(1, 8, 6)
                        and self.arm_cmd.set_protocol_identifier(3) == 0
                    ):
                        protocol_identifier = 3
                    if (
                        protocol_identifier == 3
                        and curr_time - last_send_time > 10
                        and curr_time - self.arm_cmd.last_comm_time > 30
                    ):
                        code, _ = self.get_state()
                ...
                if not self.reported:
                    ...
                    if report_socket_connected:
                        ...
                        self._report_connect_changed_callback(
                            main_socket_connected, report_socket_connected
                        )
                    self._connect_report()
                    if not self.reported:
                        connect_failed_cnt += 1
                        if self.connected and (
                            connect_failed_cnt <= max_reconnect_cnts
                            or protocol_identifier == 3
                        ):
                            time.sleep(2)
                        elif not self.connected or protocol_identifier == 2:
                            logger.error(
                                "report thread is break, connected={}, failed_cnts={}".format(
                                    self.connected, connect_failed_cnt
                                )
                            )
                            break
                        continue
    ...
    def _handle_report_data(self, data):
        def __handle_report_normal_old(rx_data):
            ...
            if error_code != self._error_code or warn_code != self._warn_code:
                if error_code != self._error_code:
                    ...
                    if self._error_code != 0:
                        pretty_print(
                            "Error, code: {}".format(self._error_code), color="red"
                        )
                    else:
                ...
                if warn_code != self._warn_code:
                    ...
                    if self._warn_code != 0:
                        pretty_print(
                            "Warn, code: {}".format(self._warn_code), color="yellow"
                        )
                    else:
                        pretty_print("Warnning had clean", color="blue")
                self._report_error_warn_changed_callback()
                logger.info(
                    "OnReport -> err={}, warn={}, state={}, cmdnum={}, mtbrake={}, mtable={}".format(
            ...
            if (
                mtbrake != self._arm_motor_brake_states
                or mtable != self._arm_motor_enable_states
            ):
            ...
            if not self._is_first_report:
                if state in [4, 5] or not all(
                    [bool(item[0] & item[1]) for item in zip(mtbrake, mtable)][
                        : self.axis
                    ]
                ):
                    # if self._is_ready:
            ...
            for i in range(len(pose)):
                pose[i] = filter_invaild_number(
                    pose[i], 3 if i < 3 else 6, default=self._position[i]
                )
            for i in range(len(angles)):
                angles[i] = filter_invaild_number(angles[i], 6, default=self._angles[i])
            for i in range(len(pose_offset)):
                pose_offset[i] = filter_invaild_number(
                    pose_offset[i], 3 if i < 3 else 6, default=self._position_offset[i]
                )
        ...
        def __handle_report_normal(rx_data):
            ...
            if (
                (length != data_len and (length != 233 or data_len != 245))
                or collis_sens not in list(range(6))
                or teach_sens not in list(range(6))
                or mode not in list(range(12))
                or state not in list(range(10))
            ):
                self._stream_report.close()
                logger.warn(
                    "ReportDataException: length={}, data_len={}, "
                ...
                return
            self._gravity_direction = convert.bytes_to_fp32s(
                rx_data[133 : 3 * 4 + 133], 3
            )
            ...
            if error_code != self._error_code or warn_code != self._warn_code:
                if error_code != self._error_code:
                    ...
                    if self._error_code != 0:
                        pretty_print(
                            "ControllerError, code: {}".format(self._error_code),
                            color="red",
                        )
                    else:
                ...
                if warn_code != self._warn_code:
                    ...
                    if self._warn_code != 0:
                        pretty_print(
                            "ControllerWarning, code: {}".format(self._warn_code),
                            color="yellow",
                        )
                    else:
                ...
                self._report_error_warn_changed_callback()
                logger.info(
                    "OnReport -> err={}, warn={}, state={}, cmdnum={}, mtbrake={}, mtable={}, mode={}".format(
            ...
            if state != self._state:
                if (
                    not self._has_motion_cmd
                    and self._state in [0, 1]
                    and state not in [0, 1]
                ):
            ...
            if compare_version(self.version_number, (0, 2, 0)):
                self._tcp_load = [
                    float("{:.3f}".format(tcp_load[0])),
                    [float("{:.3f}".format(i)) for i in tcp_load[1:]],
                ]
            else:
                self._tcp_load = [
                    float("{:.3f}".format(tcp_load[0])),
                    [float("{:.3f}".format(i * 1000)) for i in tcp_load[1:]],
                ]
        ...
        def __handle_report_rich(rx_data):
            ...
            for i in range(self.axis):
                if (
                    ...
                    or self._servo_codes[i][1] != servo_codes[i * 2 + 1]
                ):
                    print(
                        "servo_error_code, servo_id={}, status={}, code={}".format(
                            i + 1, servo_codes[i * 2], servo_codes[i * 2 + 1]
                        )
                    )
                self._servo_codes[i][0] = servo_codes[i * 2]
            ...
            if length >= 312:
                ...
                if (
                    math.inf not in world_offset
                    and -math.inf not in world_offset
                    and not (10 <= self._error_code <= 17)
                ):
            ...
            if length >= 417:
                ...
                self._is_collision_detection, self._collision_tool_type = rx_data[
                    315:317
                ]
                self._collision_tool_params = convert.bytes_to_fp32s(
                    rx_data[317:341], 6
                )
                ...
                cgpio_states[6:10] = list(
                    map(lambda x: x / 4095.0 * 10.0, cgpio_states[6:10])
                )
            ...
            if length >= 494:
                ...
                for i in range(len(pose_aa)):
                    pose_aa[i] = filter_invaild_number(
                        pose_aa[i], 6, default=self._pose_aa[i]
                    )
                self._pose_aa = self._position[:3] + pose_aa
            if length >= 495:
                self._is_reduced_mode = rx_data[494] & 0x01
                self._is_fence_mode = (rx_data[494] >> 1) & 0x01
                self._is_report_current = (
                    rx_data[494] >> 2
                ) & 0x01  # 针对get_report_tau_or_i的结果
    ...
    def _auto_get_report_thread(self):
        ...
        while self.connected:
            try:
                ...
                if error_code != self._error_code or warn_code != self._warn_code:
                    self._report_error_warn_changed_callback()
                elif not self._only_report_err_warn_changed and (
                    self._error_code != 0 or self._warn_code != 0
                ):
                    self._report_error_warn_changed_callback()
    ...
    def _set_params(self, **kwargs):
        ...
        if "A" in kwargs and isinstance(kwargs["A"], (int, float)):
            self._last_position[3] = (
                kwargs.get("A") if is_radian else math.radians(kwargs.get("A"))
            )
        if "B" in kwargs and isinstance(kwargs["B"], (int, float)):
            self._last_position[4] = (
                kwargs.get("B") if is_radian else math.radians(kwargs.get("B"))
            )
        if "C" in kwargs and isinstance(kwargs["C"], (int, float)):
            self._last_position[5] = (
                kwargs.get("C") if is_radian else math.radians(kwargs.get("C"))
            )
        ...
        if "I" in kwargs and isinstance(kwargs["I"], (int, float)):
            self._last_angles[0] = (
                kwargs.get("I") if is_radian else math.radians(kwargs.get("I"))
            )
        if "J" in kwargs and isinstance(kwargs["J"], (int, float)):
            self._last_angles[1] = (
                kwargs.get("J") if is_radian else math.radians(kwargs.get("J"))
            )
        if "K" in kwargs and isinstance(kwargs["K"], (int, float)):
            self._last_angles[2] = (
                kwargs.get("K") if is_radian else math.radians(kwargs.get("K"))
            )
        if "L" in kwargs and isinstance(kwargs["L"], (int, float)):
            self._last_angles[3] = (
                kwargs.get("L") if is_radian else math.radians(kwargs.get("L"))
            )
        if "M" in kwargs and isinstance(kwargs["M"], (int, float)):
            self._last_angles[4] = (
                kwargs.get("M") if is_radian else math.radians(kwargs.get("M"))
            )
        if "N" in kwargs and isinstance(kwargs["N"], (int, float)):
            self._last_angles[5] = (
                kwargs.get("N") if is_radian else math.radians(kwargs.get("N"))
            )
        if "O" in kwargs and isinstance(kwargs["O"], (int, float)):
            self._last_angles[6] = (
                kwargs.get("O") if is_radian else math.radians(kwargs.get("O"))
            )

        if "F" in kwargs and isinstance(kwargs["F"], (int, float)):
            self._last_tcp_speed = kwargs.get("F")
            self._last_tcp_speed = min(
                max(self._last_tcp_speed, self._min_tcp_speed), self._max_tcp_speed
            )
        if "Q" in kwargs and isinstance(kwargs["Q"], (int, float)):
            self._last_tcp_acc = kwargs.get("Q")
            self._last_tcp_acc = min(
                max(self._last_tcp_acc, self._min_tcp_acc), self._max_tcp_acc
            )
        if "F2" in kwargs and isinstance(kwargs["F2"], (int, float)):
            ...
            self._last_joint_speed = min(
                max(self._last_joint_speed, self._min_joint_speed),
                self._max_joint_speed,
            )
        if "Q2" in kwargs and isinstance(kwargs["Q2"], (int, float)):
            ...
            self._last_joint_acc = min(
                max(self._last_joint_acc, self._min_joint_acc), self._max_joint_acc
            )
        if "T" in kwargs and isinstance(kwargs["T"], (int, float)):
            self._mvtime = kwargs.get("T")
        if (
            "LIMIT_VELO" in kwargs
            and isinstance(kwargs["LIMIT_VELO"], (list, tuple))
            and len(kwargs["LIMIT_VELO"]) == 2
            and isinstance(kwargs["LIMIT_VELO"][0], (int, float))
        ...
        if (
            "LIMIT_ACC" in kwargs
            and isinstance(kwargs["LIMIT_ACC"], (list, tuple))
            and len(kwargs["LIMIT_ACC"]) == 2
            and isinstance(kwargs["LIMIT_ACC"][0], (int, float))
    ...
    def _get_params(self, is_radian=None):
        ...
        if is_radian:
            ...
            return {
                ...
                "mvtime": self._mvtime,
                "LIMIT_VELO": list(
                    map(round, [self._min_tcp_speed, self._max_tcp_speed])
                ),
                "LIMIT_ACC": list(map(round, [self._min_tcp_acc, self._max_tcp_acc])),
                "LIMIT_ANGLE_VELO": list(
                    map(
                        round,
                        [
                            math.degrees(self._min_joint_speed),
                            math.degrees(self._max_joint_speed),
                        ],
                    )
                ),
                "LIMIT_ANGLE_ACC": list(
                    map(
                        round,
                        [
                            math.degrees(self._min_joint_acc),
                            math.degrees(self._max_joint_acc),
                        ],
                    )
                ),
            }
            ...
            if code in [0, XCONF.UxbusState.WAR_CODE]:
                if self.arm_cmd.state_is_ready:
                    if mode >= 0 and mode != self.mode:
                        logger.warn(
                            "The mode may be incorrect, just as a reminder, mode: {} ({})".format(
                                mode, self.mode
                            )
                        )
                    return 0
    ...
    def get_robot_sn(self):
        ...
        if ret[0] == 0:
            ...
            self._arm_type_is_1300 = (
                int(self._robot_sn[2:6]) >= 1300
                if self._robot_sn[2:6].isdigit()
                else False
            )
            self._control_box_type_is_1300 = (
                int(self._control_box_sn[2:6]) >= 1300
                if self._control_box_sn[2:6].isdigit()
                else False
            )
    ...
    def get_position(self, is_radian=None):
        ...
        if ret[0] == 0 and len(ret) > 6:
            self._position = [
                filter_invaild_number(ret[i], 6, default=self._position[i - 1])
                for i in range(1, 7)
            ]
        return ret[0], [
            float(
    ...
    def get_servo_angle(self, servo_id=None, is_radian=None, is_real=False):
        ...
        if ret[0] == 0 and len(ret) > 7:
            self._angles = [
                filter_invaild_number(ret[i], 6, default=self._angles[i - 1])
                for i in range(1, 8)
            ]
        if servo_id is None or servo_id == 8 or len(self._angles) < servo_id:
            return ret[0], list(
                map(
                    lambda x: float(
                        "{:.6f}".format(x if is_radian else math.degrees(x))
                    ),
                    self._angles,
                )
            )
            ...
            return ret[0], float(
                "{:.6f}".format(
                    self._angles[servo_id - 1]
                    if is_radian
                    else math.degrees(self._angles[servo_id - 1])
                )
            )
    ...
    def get_position_aa(self, is_radian=None):
        ...
        if ret[0] == 0 and len(ret) > 6:
            self._pose_aa = [
                filter_invaild_number(ret[i], 6, default=self._pose_aa[i - 1])
                for i in range(1, 7)
            ]
        return ret[0], [
            float(
    ...
    @xarm_is_connected(_type="get")
    def get_pose_offset(
        self, pose1, pose2, orient_type_in=0, orient_type_out=0, is_radian=None
    ):
        is_radian = self._default_is_radian if is_radian is None else is_radian
        _pose1 = [
            pose1[i] if i <= 2 or is_radian else math.radians(pose1[i])
            for i in range(6)
        ]
        _pose2 = [
            pose2[i] if i <= 2 or is_radian else math.radians(pose2[i])
            for i in range(6)
        ]
        ret = self.arm_cmd.get_pose_offset(
            _pose1, _pose2, orient_type_in, orient_type_out
        )
        ret[0] = self._check_code(ret[0])
        if ret[0] == 0 and len(ret) > 6:
            pose = [
                float(
                    "{:.6f}".format(
                        ret[i] if i <= 3 or is_radian else math.degrees(ret[i])
                    )
                )
                for i in range(1, 7)
            ]
    ...
    def set_state(self, state=0):
        ...
        if self._state in [4, 5]:
            ...
            self._is_ready = True
        self.log_api_info(
            "API -> set_state({}) -> code={}, state={}".format(
                state, ret[0], self._state
            ),
            code=ret[0],
        )
        return ret[0]
    ...
    def set_mode(self, mode=0, detection_param=0):
        ...
        ret[0] = self._check_code(ret[0])
        self.log_api_info(
            "API -> set_mode({}) -> code={}".format(mode, ret[0]), code=ret[0]
        )
        return ret[0]
    ...
    def _gen_feedback_key(self, wait, **kwargs):
        feedback_key = (
            kwargs.get("feedback_key", "")
            if self._support_feedback and not wait
            else ""
        )
        studio_wait = bool(feedback_key)
        feedback_key = (
            str(uuid.uuid1()) if wait and self._support_feedback else feedback_key
        )
    ...
    def _wait_feedback(self, timeout=None, trans_id=-1, ignore_log=False):
        if timeout is not None:
            expired = (
                ...
                + timeout
                + (
                    self._sleep_finish_time
                    if self._sleep_finish_time > time.monotonic()
                    else 0
                )
            )
        else:
        ...
        while timeout is None or time.monotonic() < expired:
            if not self.connected:
                ...
                if not ignore_log:
                    self.log_api_info(
                        "wait_feedback, xarm is disconnect", code=APIState.NOT_CONNECTED
                    )
            ...
            if self.error_code != 0:
                ...
                if not ignore_log:
                    self.log_api_info(
                        "wait_feedback, xarm has error, error={}".format(
                            self.error_code
                        ),
                        code=APIState.HAS_ERROR,
                    )
            ...
            if state >= 4:
                ...
                if state != 5 or state5_cnt >= 20:
                    ...
                    if not ignore_log:
                        self.log_api_info(
                            "wait_feedback, xarm is stop, state={}".format(state),
    ...
    def wait_move(self, timeout=None, trans_id=-1):
        ...
        while timeout is None or time.monotonic() < expired:
            if not self.connected:
                self.log_api_info(
                    "wait_move, xarm is disconnect", code=APIState.NOT_CONNECTED
                )
                return APIState.NOT_CONNECTED
            if self.error_code != 0:
                self.log_api_info(
                    "wait_move, xarm has error, error={}".format(self.error_code),
                    code=APIState.HAS_ERROR,
                )
            ...
            if state >= 4:
                ...
                if state != 5 or state5_cnt >= 20:
                    self.log_api_info(
                        "wait_move, xarm is stop, state={}".format(state),
                        code=APIState.EMERGENCY_STOP,
                    )
    ...
    @xarm_is_connected(_type="set")
    def _check_modbus_code(
        self, ret, length=2, only_check_code=False, host_id=XCONF.TGPIO_HOST_ID
    ):
        code = ret[0]
    ...
    def checkset_modbus_baud(self, baudrate, check=True, host_id=XCONF.TGPIO_HOST_ID):
        if check and (not self._baud_checkset or baudrate <= 0):
            return 0
        if check and (
            (host_id == XCONF.TGPIO_HOST_ID and self.modbus_baud == baudrate)
            or (
                host_id == XCONF.LINEER_TRACK_HOST_ID
                and self.linear_track_baud == baudrate
            )
        ):
            return 0
            ...
            if cur_baud_inx != baud_inx:
                try:
                    ...
                    if host_id != XCONF.LINEER_TRACK_HOST_ID:
                        self.arm_cmd.tgpio_addr_w16(
                            XCONF.ServoConf.SOFT_REBOOT, 1, bid=host_id
                        )
                    if host_id == XCONF.TGPIO_HOST_ID:
                        if self.error_code != 19 and self.error_code != 28:
                            self.get_err_warn_code()
                ...
                ret, cur_baud_inx = self._get_modbus_baudrate_inx(host_id=host_id)
                self.log_api_info(
                    "API -> checkset_modbus_baud -> code={}, baud_inx={}".format(
                        ret, cur_baud_inx
                    ),
                    code=ret,
                )
        ...
        if host_id == XCONF.TGPIO_HOST_ID:
            return (
                0 if self.modbus_baud == baudrate else APIState.MODBUS_BAUD_NOT_CORRECT
            )
        elif host_id == XCONF.LINEER_TRACK_HOST_ID:
            return (
                0
                if self.linear_track_baud == baudrate
                else APIState.MODBUS_BAUD_NOT_CORRECT
            )
        else:
            if ret == 0 and 0 <= cur_baud_inx < len(self.arm_cmd.BAUDRATES):
                return (
                    0
                    if self.arm_cmd.BAUDRATES[cur_baud_inx] == baudrate
                    else APIState.MODBUS_BAUD_NOT_CORRECT
                )
    ...
    def _get_modbus_baudrate_inx(self, host_id=XCONF.TGPIO_HOST_ID):
        ret = self.arm_cmd.tgpio_addr_r16(
            XCONF.ServoConf.MODBUS_BAUDRATE & 0x0FFF, bid=host_id
        )
    ...
    @xarm_is_connected(_type="set")
    def set_tgpio_modbus_timeout(
        self, timeout, is_transparent_transmission=False, **kwargs
    ):
        ret = self.arm_cmd.set_modbus_timeout(
            timeout,
            is_transparent_transmission=kwargs.get(
                "is_tt", is_transparent_transmission
            ),
        )
        self.log_api_info(
            "API -> set_tgpio_modbus_timeout -> code={}".format(ret[0]), code=ret[0]
        )
        return ret[0]
    ...
    def set_tgpio_modbus_baudrate(self, baud):
        ...
        self.log_api_info(
            "API -> set_tgpio_modbus_baudrate -> code={}".format(code), code=code
        )
        return code
    ...
    def getset_tgpio_modbus_data(
        ...
        **kwargs
        ...
        if is_tt:
            if use_503_port:
                if not self.connected_503 and self.connect_503() != 0:
                    return APIState.NOT_CONNECTED, []
                ret = self.arm_cmd_503.tgpio_set_modbus(
                    datas, len(datas), host_id=host_id, is_transparent_transmission=True
                )
            else:
                ret = self.arm_cmd.tgpio_set_modbus(
                    datas, len(datas), host_id=host_id, is_transparent_transmission=True
                )
        else:
        ...
        if not ignore_log:
            self.log_api_info(
                "API -> getset_tgpio_modbus_data -> code={}, response={}".format(
                    ret[0], ret[2:]
                ),
                code=ret[0],
            )
    ...
    def set_simulation_robot(self, on_off):
        ...
        ret[0] = self._check_code(ret[0])
        self.log_api_info(
            "API -> set_simulation_robot({}) -> code={}".format(on_off, ret[0]),
            code=ret[0],
        )
        return ret[0]
    ...
    def set_tcp_load(self, weight, center_of_gravity, wait=False, **kwargs):
        ...
        ret = self.arm_cmd.set_tcp_load(
            weight, _center_of_gravity, feedback_key=feedback_key
        )
        trans_id = self._get_feedback_transid(feedback_key, studio_wait)
        ret[0] = self._check_code(ret[0], is_move_cmd=True)
        self.log_api_info(
            "API -> set_tcp_load -> code={}, weight={}, center={}".format(
                ret[0], weight, _center_of_gravity
            ),
            code=ret[0],
        )
        if wait and ret[0] == 0:
    ...
    def set_only_check_type(self, only_check_type):
        self._only_check_type = (
            only_check_type if only_check_type in [0, 1, 2, 3] else 0
        )
