...
class FtSensor(Base):
            ...
            for i in range(6):
                if i < 3:
                    if M[i] < 0.02 or M[i] > 1.0:
                        logger.error(
                            "set_impedance, M[{}] over range, range=[0.02, 1.0]".format(
                                i
                            )
                        )
                        return APIState.API_EXCEPTION
                    if K[i] < 0 or K[i] > 2000:
                        logger.error(
                            "set_impedance, K[{}] over range, range=[0, 2000]".format(i)
                        )
                        return APIState.API_EXCEPTION
                else:
                    if M[i] < 0.0001 or M[i] > 0.01:
                        logger.error(
                            "set_impedance, M[{}] over range, range=[0.0001, 0.01]".format(
                                i
                            )
                        )
                        return APIState.API_EXCEPTION
                    if K[i] < 0 or K[i] > 20:
                        logger.error(
                            "set_impedance, K[{}] over range, range=[0, 20]".format(i)
                        )
                        return APIState.API_EXCEPTION
                if B[i] < 0:
                    logger.error(
                        "set_impedance, the value of B[{}] must be greater than or equal to 0".format(
                            i
                        )
                    )
                    return APIState.API_EXCEPTION
    ...
    def set_impedance_mbk(self, M, K, B, **kwargs):
        ...
        if params_limit:
            for i in range(6):
                if i < 3:
                    if M[i] < 0.02 or M[i] > 1.0:
                        logger.error(
                            "set_impedance_mbk, M[{}] over range, range=[0.02, 1.0]".format(
                                i
                            )
                        )
                        return APIState.API_EXCEPTION
                    if K[i] < 0 or K[i] > 2000:
                        logger.error(
                            "set_impedance_mbk, K[{}] over range, range=[0, 2000]".format(
                                i
                            )
                        )
                        return APIState.API_EXCEPTION
                    ...
                    if M[i] < 0.0001 or M[i] > 0.01:
                        logger.error(
                            "set_impedance_mbk, M[{}] over range, range=[0.0001, 0.01]".format(
                                i
                            )
                        )
                        return APIState.API_EXCEPTION
                    if K[i] < 0 or K[i] > 20:
                        logger.error(
                            "set_impedance_mbk, K[{}] over range, range=[0, 20]".format(
                                i
                            )
                        )
                        return APIState.API_EXCEPTION
                if B[i] < 0:
                    logger.error(
                        "set_impedance_mbk, the value of B[{}] must be greater than or equal to 0".format(
                            i
                        )
                    )
                    return APIState.API_EXCEPTION
        ...
        self.log_api_info(
            "API -> set_impedance_mbk -> code={}".format(ret[0]), code=ret[0]
        )
        return self._check_code(ret[0])
    ...
    def set_impedance_config(self, coord, c_axis):
        ...
        self.log_api_info(
            "API -> set_impedance_config -> code={}".format(ret[0]), code=ret[0]
        )
        return self._check_code(ret[0])
        ...
        if params_limit:
            ...
            for i in range(6):
                if f_ref[i] < -max_f_ref[i] or f_ref[i] > max_f_ref[i]:
                    logger.error(
                        "config_force_control, f_ref[{}] over range, range=[{}, {}]".format(
                            i, -max_f_ref[i], max_f_ref[i]
                        )
                    )
                    return APIState.API_EXCEPTION
        ret = self.arm_cmd.config_force_control(coord, c_axis, f_ref, limits)
        self.log_api_info(
            "API -> config_force_control -> code={}".format(ret[0]), code=ret[0]
        )
        return self._check_code(ret[0])
    ...
    def set_force_control_pid(self, kp, ki, kd, xe_limit, **kwargs):
        ...
        if params_limit:
            for i in range(6):
                if kp[i] < 0 or kp[i] > 0.05:
                    logger.error(
                        "set_force_control_pid, kp[{}] over range, range=[0, 0.05]".format(
                            i
                        )
                    )
                    return APIState.API_EXCEPTION
                if ki[i] < 0 or ki[i] > 0.0005:
                    logger.error(
                        "set_force_control_pid, ki[{}] over range, range=[0, 0.0005]".format(
                            i
                        )
                    )
                    return APIState.API_EXCEPTION
                if kd[i] < 0 or kd[i] > 0.05:
                    logger.error(
                        "set_force_control_pid, kd[{}] over range, range=[0, 0.05]".format(
                            i
                        )
                    )
                    return APIState.API_EXCEPTION
                if xe_limit[i] < 0 or xe_limit[i] > 200:
                    logger.error(
                        "set_force_control_pid, xe_limit[{}] over range, range=[0, 200]".format(
                            i
                        )
                    )
                    return APIState.API_EXCEPTION
        ...
        self.log_api_info(
            "API -> set_force_control_pid -> code={}".format(ret[0]), code=ret[0]
        )
        return self._check_code(ret[0])
    ...
    def ft_sensor_set_zero(self):
        ...
        self.log_api_info(
            "API -> ft_sensor_set_zero -> code={}".format(ret[0]), code=ret[0]
        )
        return self._check_code(ret[0])
    ...
    def ft_sensor_iden_load(self):
        ...
        self.log_api_info(
            "API -> ft_sensor_iden_load -> code={}".format(ret[0]), code=ret[0]
        )
    ...
    @xarm_is_connected(_type="set")
    def ft_sensor_cali_load(
        self, iden_result_list, association_setting_tcp_load=False, **kwargs
    ):
        ...
        self.log_api_info(
            "API -> ft_sensor_cali_load -> code={}, iden_result_list={}".format(
                ret[0], iden_result_list
            ),
            code=ret[0],
        )
    ...
    def ft_sensor_enable(self, on_off):
        ...
        self.log_api_info(
            "API -> ft_sensor_enable -> code={}, on_off={}".format(ret[0], on_off),
            code=ret[0],
        )
        return self._check_code(ret[0])
    ...
    def ft_sensor_app_set(self, app_code):
        ...
        self.log_api_info(
            "API -> ft_sensor_app_set -> code={}, app_code={}".format(ret[0], app_code),
            code=ret[0],
        )
        return self._check_code(ret[0])
    ...
    def set_ft_sensor_sn(self, sn):
        ...
        if len(sn) == 14:
            for i in range(0, 14):
                ...
                time.sleep(0.05)
        self.log_api_info(
            "API -> set_ft_sensor_sn -> code={}, sn={}".format(ret[0], sn), code=ret[0]
        )
        return ret[0]
    ...
    def get_ft_sensor_sn(self):
        ...
        for i in range(0, 14):
            ...
            time.sleep(0.05)
        self.log_api_info(
            "API -> get_ft_sensor_sn -> code={}, sn={}".format(ret[0], rd_sn),
            code=ret[0],
        )
        return ret[0], rd_sn
