...
class _BlocklyHandler(_BlocklyBase):
                    ...
                    if self._highlight_callback is not None:
                        if block.attrib["type"] in HIGHLIGHT_BLOCKS:
                            self._append_main_code(
                                "highlight_callback('{}')".format(block.attrib["id"]),
                                indent=indent + 2,
                            )
                    try:
    ...
    def _handle_set_speed(self, block, indent=0, arg_map=None):
        self._append_main_code(
            "self._tcp_speed = {}".format(self._get_field_value(block)), indent + 2
        )

    def _handle_set_acceleration(self, block, indent=0, arg_map=None):
        self._append_main_code(
            "self._tcp_acc = {}".format(self._get_field_value(block)), indent + 2
        )

    def _handle_set_angle_speed(self, block, indent=0, arg_map=None):
        self._append_main_code(
            "self._angle_speed = {}".format(self._get_field_value(block)), indent + 2
        )

    def _handle_set_angle_acceleration(self, block, indent=0, arg_map=None):
        self._append_main_code(
            "self._angle_acc = {}".format(self._get_field_value(block)), indent + 2
        )
    ...
    def _handle_set_counter_increase(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "if not self._check_code(code, 'set_counter_increase'):", indent + 2
        )
    ...
    def _handle_set_counter_reset(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "if not self._check_code(code, 'set_counter_reset'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_sleep(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "code = self._arm.set_pause_time({})".format(value), indent + 2
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_pause_time'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_move(self, block, indent=0, arg_map=None):
        ...
        value = (
            fields[1].text
            if orientation == "forward" or orientation == "left" or orientation == "up"
            else "-{}".format(fields[1].text)
        )
        ...
        param = (
            ...
            if orientation == "forward" or orientation == "backward"
                ...
                else "z" if orientation == "up" or orientation == "down" else None
            )
        )
        if param is None:
            return
        ...
        self._append_main_code(
            "code = self._arm.set_position({}={}, radius={}, speed=self._tcp_speed, mvacc=self._tcp_acc, relative=True, wait={})".format(
                param, value, radius, wait
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_position'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_move_arc_to(self, block, indent=0, arg_map=None):
        ...
        wait = fields[-1].text == "TRUE"
        self._append_main_code(
            "code = self._arm.set_position(*{}, speed=self._tcp_speed, mvacc=self._tcp_acc, radius={}, wait={})".format(
                values, radius, wait
            ),
            indent + 2,
        )
    ...
    def _handle_move_circle(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "code = self._arm.move_circle({}, {}, float({}) / 360 * 100, speed=self._tcp_speed, mvacc=self._tcp_acc, wait={})".format(
                pose1, pose2, percent, wait
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'move_circle'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_move_7(self, block, indent=0, arg_map=None):
        ...
        wait = fields[-1].text == "TRUE"
        self._append_main_code(
            "code = self._arm.set_servo_angle(angle={}, speed=self._angle_speed, mvacc=self._angle_acc, wait={})".format(
                values, wait
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_servo_angle'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_move_joints(self, block, indent=0, arg_map=None):
        ...
        wait = fields[-1].text == "TRUE"
        self._append_main_code(
            "code = self._arm.set_servo_angle(angle={}, speed=self._angle_speed, mvacc=self._angle_acc, wait={}, radius={})".format(
                values, wait, radius
            ),
            indent + 2,
        )
    ...
    def _handle_move_tool_line(self, block, indent=0, arg_map=None):
        ...
        wait = fields[-1].text == "TRUE"
        self._append_main_code(
            "code = self._arm.set_tool_position(*{}, speed=self._tcp_speed, mvacc=self._tcp_acc, wait={})".format(
                values, wait
            ),
            indent + 2,
        )
    ...
    def _handle_motion_set_state(self, block, indent=0, arg_map=None):
        ...
        state = fields[0].text
        self._append_main_code(
            "code = self._arm.set_state({})".format(state), indent + 2
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_state'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_studio_run_traj(self, block, indent=0, arg_map=None):
        ...
        times = fields[2].text
        self._append_main_code(
            "code = self._arm.playback_trajectory(times={}, filename='{}', wait=True, double_speed={})".format(
                times, filename, speed
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'playback_trajectory'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_tool_message(self, block, indent=0, arg_map=None):
        ...
        color = (
            json.dumps(fields[0].text, ensure_ascii=False)
            if len(fields) > 1
            else "white"
        )
        msg = json.dumps(
            fields[1].text if fields[-1].text is not None else "", ensure_ascii=False
        )
        if self._highlight_callback is not None:
    ...
    def _handle_tool_console_with_variable(self, block, indent=0, arg_map=None):
        ...
        if msg:
            if self._highlight_callback is not None:
                self._append_main_code(
                    "print({}.format({}), color={})".format(
                        json.dumps(msg + "{}", ensure_ascii=False), expression, color
                    ),
                    indent + 2,
                )
            else:
                self._append_main_code(
                    "print({}.format({}))".format(
                        json.dumps(msg + "{}", ensure_ascii=False), expression
                    ),
                    indent + 2,
                )
        else:
            if self._highlight_callback is not None:
                self._append_main_code(
                    "print('{{}}'.format({}), color={})".format(expression, color),
                    indent + 2,
                )
            else:
                self._append_main_code(
                    "print('{{}}'.format({}))".format(expression), indent + 2
                )
    ...
    def _handle_gpio_set_digital(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "code = self._arm.set_tgpio_digital({}, {}, delay_sec={})".format(
                io, value, delay_sec
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_tgpio_digital'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_gpio_set_digital_with_xyz(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "code = self._arm.set_tgpio_digital_with_xyz({}, {}, {}, {})".format(
                io, value, xyz, tol_r
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_tgpio_digital_with_xyz'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_check_air_pump_state(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "self._arm.arm.check_air_pump_state({}, timeout={})".format(state, timeout),
            indent + 2,
        )
    ...
    def _handle_check_bio_gripper_is_catch(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "self._arm.arm.check_bio_gripper_is_catch(timeout={})".format(timeout),
            indent + 2,
        )
    ...
    def _handle_check_robotiq_is_catch(self, block, indent=0, arg_map=None):
        ...
        timeout = float(fields[0].text)
        self._append_main_code(
            "self._arm.arm.check_robotiq_is_catch(timeout={})".format(timeout),
            indent + 2,
        )
    ...
    def _handle_set_suction_cup(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "code = self._arm.set_suction_cup({}, wait={}, delay_sec={})".format(
                on, wait, delay_sec
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_suction_cup'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_gpio_get_controller_ci_li(self, block, indent=0, arg_map=None):
        ...
        timeout = float(fields[-1].text)
        self._append_main_code(
            "self._arm.arm.get_cgpio_li_state({},timeout={},is_ci=True)".format(
                values, timeout
            ),
            indent + 2,
        )
    ...
    def _handle_gpio_get_controller_di_li(self, block, indent=0, arg_map=None):
        ...
        timeout = float(fields[-1].text)
        self._append_main_code(
            "self._arm.arm.get_cgpio_li_state({},timeout={},is_ci=False)".format(
                values, timeout
            ),
            indent + 2,
        )
    ...
    def _handle_gpio_get_tgpio_li(self, block, indent=0, arg_map=None):
        ...
        timeout = float(fields[-1].text)
        self._append_main_code(
            "self._arm.arm.get_tgpio_li_state({},timeout={})".format(values, timeout),
            indent + 2,
        )
    ...
    def _handle_gpio_set_controller_digital(self, block, indent=0, arg_map=None):
        ...
        delay_sec = fields[2].text if len(fields) > 2 else 0
        self._append_main_code(
            "code = self._arm.set_cgpio_digital({}, {}, delay_sec={})".format(
                io, value, delay_sec
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_cgpio_digital'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)

    def _handle_gpio_set_controller_digital_with_xyz(
        self, block, indent=0, arg_map=None
    ):
        fields = self._get_nodes("field", root=block)
        x = fields[0].text
        y = fields[1].text
        ...
        value = 0 if fields[5].text == "LOW" else 1
        self._append_main_code(
            "code = self._arm.set_cgpio_digital_with_xyz({}, {}, {}, {})".format(
                io, value, xyz, tol_r
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_cgpio_digital_with_xyz'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_gpio_set_controller_analog_with_xyz(
        ...
        value = fields[5].text
        self._append_main_code(
            "code = self._arm.set_cgpio_analog_with_xyz({}, {}, {}, {})".format(
                io, value, xyz, tol_r
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_cgpio_analog_with_xyz'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_gpio_set_controller_analog(self, block, indent=0, arg_map=None):
        ...
        value = self._get_block_val(value, arg_map=arg_map)
        self._append_main_code(
            "code = self._arm.set_cgpio_analog({}, {})".format(io, value), indent + 2
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_cgpio_analog'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_set_collision_sensitivity(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "code = self._arm.set_collision_sensitivity({})".format(value), indent + 2
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_collision_sensitivity'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_set_teach_sensitivity(self, block, indent=0, arg_map=None):
        ...
        value = self._get_nodes("field", root=value, descendant=True)[0].text
        self._append_main_code(
            "code = self._arm.set_teach_sensitivity({})".format(value), indent + 2
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_teach_sensitivity'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_set_tcp_load(self, block, indent=0, arg_map=None):
        ...
        z = fields[4].text
        self._append_main_code(
            "code = self._arm.set_tcp_load({}, [{}, {}, {}])".format(weight, x, y, z),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_tcp_load'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_set_gravity_direction(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "code = self._arm.set_gravity_direction([{}, {}, {}])".format(x, y, z),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_gravity_direction'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_set_tcp_offset(self, block, indent=0, arg_map=None):
        ...
        yaw = fields[6].text
        self._append_main_code(
            "code = self._arm.set_tcp_offset([{}, {}, {}, {}, {}, {}], wait=True)".format(
        ...
        self._append_main_code(
            "if not self._check_code(code, 'set_tcp_offset'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_set_world_offset(self, block, indent=0, arg_map=None):
        ...
        yaw = fields[6].text
        self._append_main_code(
            "code = self._arm.set_world_offset([{}, {}, {}, {}, {}, {}])".format(
                x, y, z, roll, pitch, yaw
            ),
            indent + 2,
        )
        self._append_main_code("self._arm.set_state(0)", indent + 2)
        self._append_main_code(
            "if not self._check_code(code, 'set_world_offset'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
        self._append_main_code("time.sleep(0.5)", indent + 2)
    ...
    def _handle_set_lite6_gripper(self, block, indent=0, arg_map=None):
        ...
        if op == "OPEN":
            ...
            return
        self._append_main_code(
            "if not self._check_code(code, '{}'):".format(func), indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_gripper_set(self, block, indent=0, arg_map=None):
        ...
        if fields is not None and len(fields) >= 3:
            ...
            wait = (
                self._get_nodes("field", root=values[2], descendant=True)[0].text
                == "TRUE"
            )
        self._append_main_code(
            "code = self._arm.set_gripper_position({}, wait={}, speed={}, auto_enable=True)".format(
                pos, wait, speed
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_gripper_position'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_gripper_set_status(self, block, indent=0, arg_map=None):
        ...
        delay_sec = fields[0].text if len(fields) > 0 else 0
        self._append_main_code(
            "code = self._arm.arm.set_gripper_status({}, delay_sec={})".format(
                status, delay_sec
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_gripper_status'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)

    def _handle_set_bio_gripper_init(self, block, indent=0, arg_map=None):
        self._append_main_code(
            "code = self._arm.set_bio_gripper_enable(True)", indent + 2
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_bio_gripper_enable'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_set_bio_gripper(self, block, indent=0, arg_map=None):
        ...
        if on:
            self._append_main_code(
                "code = self._arm.open_bio_gripper(speed={}, wait={})".format(
                    speed, wait
                ),
                indent + 2,
            )
            self._append_main_code(
                "if not self._check_code(code, 'open_bio_gripper'):", indent + 2
            )
            self._append_main_code("    return", indent + 2)
        else:
            self._append_main_code(
                "code = self._arm.close_bio_gripper(speed={}, wait={})".format(
                    speed, wait
                ),
                indent + 2,
            )
            self._append_main_code(
                "if not self._check_code(code, 'close_bio_gripper'):", indent + 2
            )
            self._append_main_code("    return", indent + 2)
    ...
    def _handle_set_robotiq_init(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "if not self._check_code(code, 'robotiq_reset'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
        self._append_main_code(
            "code = self._arm.robotiq_set_activate(wait=True)", indent + 2
        )
        self._append_main_code(
            "if not self._check_code(code, 'robotiq_set_activate'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_set_robotiq_gripper(self, block, indent=0, arg_map=None):
        ...
        wait = fields[0].text == "TRUE" if fields and len(fields) > 0 else False
        self._append_main_code(
            "code = self._arm.robotiq_set_position({}, speed={}, force={}, wait={})".format(
                pos, speed, force, wait
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'robotiq_set_position'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def __handle_gpio_event(self, gpio_type, block, indent=0, arg_map=None):
        if gpio_type.startswith("listen"):
            ...
            if gpio_type == "tgpio_digital":
                ...
                self._append_main_code(
                    "# Define Tool GPIO-{} DIGITAL is {} callback".format(io, trigger),
                    indent=1,
                )
                ...
                self._append_main_code(
                    "# Define Tool GPIO-{} ANALOG is changed callback".format(io),
                    indent=1,
                )
                ...
                name = "controller_gpio_{}_analog_is_changed_callback_{}".format(
                    io, num
                )
                self._append_main_code(
                    "# Define Contoller GPIO-{} ANALOG is changed callback".format(io),
                    indent=1,
                )
            else:
                self._is_main_run_code = True
            ...
            if gpio_type == "tgpio_digital":
                ...
                self._append_main_code(
                    "self._tgpio_digital_callbacks.append({{'io': {}, 'trigger': {}, 'op': '{}', 'callback': self.{}}})".format(
                        io, 1 if trigger == "HIGH" else 0, "==", name
                    ),
                    indent=indent + 2,
                )
            elif gpio_type == "tgpio_analog":
                ...
                self._append_main_code(
                    "self._tgpio_analog_callbacks.append({{'io': {}, 'trigger': {}, 'op': '{}', 'callback': self.{}}})".format(
                        io, trigger, op, name
                    ),
                    indent=indent + 2,
                )
                ...
                self._append_main_code(
                    "self._cgpio_digital_callbacks.append({{'io': {}, 'trigger': {}, 'op': '{}', 'callback': self.{}}})".format(
                        io, 1 if trigger == "HIGH" else 0, "==", name
                    ),
                    indent=indent + 2,
                )
                ...
                self._append_main_code(
                    "self._cgpio_analog_callbacks.append({{'io': {}, 'trigger': {}, 'op': '{}', 'callback': self.{}}})".format(
                        io, trigger, op, name
                    ),
                    indent=indent + 2,
                )
    ...
    def _handle_procedures_defnoreturn(self, block, indent=0, arg_map=None):
        ...
        try:
            ...
            if not args:
                ...
                self._append_main_code(
                    "    def {}(self, {}):".format(name, args_str), indent=indent
                )
    ...
    def _handle_procedures_defreturn(self, block, indent=0, arg_map=None):
        arg_map_ = self._handle_procedures_defnoreturn(
            block, indent=indent, arg_map=arg_map
        )
        value = self._get_node("value", root=block)
        ...
        if args and values and len(args) == len(values):
            self._append_main_code(
                "self.{}({})".format(
                    ...
                    ",".join(
                        [
                            self._get_condition_expression(val, arg_map=arg_map)
                            for val in values
                        ]
                    ),
                ),
    ...
    def _handle_variables_set(self, block, indent=0, arg_map=None):
        ...
        if arg_map and field in arg_map:
            self._append_main_code(
                "{} = {}".format(arg_map[field], expression), indent=indent + 2
            )
        else:
            self._append_main_code(
                "self._vars['{}'] = {}".format(field, expression), indent=indent + 2
            )
    ...
    def _handle_math_change(self, block, indent=0, arg_map=None):
        ...
        if arg_map and field in arg_map:
            self._append_main_code(
                "{} += {}".format(arg_map[field], val), indent=indent + 2
            )
        else:
            self._append_main_code(
                "self._vars['{}'] += {}".format(field, val), indent=indent + 2
            )
    ...
    def _handle_controls_repeat_ext(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "for i in range(int({})):".format(times), indent=indent + 2
        )
        self._append_main_code("    if not self.is_alive:", indent=indent + 2)
        ...
        if statement:
            ...
            if self._loop_interval_sec > 0:
                # limit frequency
                self._append_main_code(
                    "    interval = time.monotonic() - t1", indent=indent + 2
                )
                self._append_main_code(
                    "    if interval < {}:".format(self._loop_interval_sec),
                    indent=indent + 2,
                )
                self._append_main_code(
                    "        time.sleep({} - interval)".format(self._loop_interval_sec),
                    indent=indent + 2,
                )
        else:
            if self._loop_interval_sec > 0:
                self._append_main_code(
                    "    time.sleep({})".format(self._loop_interval_sec),
                    indent=indent + 2,
                )
            else:
    ...
    def _handle_controls_whileUntil(self, block, indent=0, arg_map=None):
        ...
        if field.text == "WHILE":
            ...
            expression = self._get_condition_expression(value, arg_map=arg_map)
            self._append_main_code(
                "while self.is_alive and {}:".format(expression), indent=indent + 2
            )
        elif field.text == "UNTIL":
            value = self._get_node("value", root=block)
            expression = self._get_condition_expression(value, arg_map=arg_map)
            self._append_main_code(
                "while self.is_alive and not {}:".format(expression), indent=indent + 2
            )
        statement = self._get_node("statement", root=block)
        if statement:
            if self._loop_interval_sec > 0:
    ...
    def _handle_controls_if(self, block, indent=0, arg_map=None):
        ...
        for st in statements:
            if st.attrib["name"] == "ELSE":
                ...
                self._parse_block(
                    st, old_indent if not has_if else indent, arg_map=arg_map
                )
                break
    ...
    def _handle_set_line_track(self, block, indent=0, arg_map=None):
        ...
        wait = True
        self._append_main_code(
            "code = self._arm.set_linear_track_pos({}, speed={}, wait={}, auto_enable=True)".format(
                pos, speed, wait
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_linear_track_pos'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)

    def _handle_set_line_track_origin(self, block, indent=0, arg_map=None):
        self._append_main_code(
            "code = self._arm.set_linear_track_back_origin(wait=True, auto_enable=True)",
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_linear_track_back_origin'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_python_code(self, block, indent=0, arg_map=None, **kwargs):
        ...
        for i, code in enumerate(codes):
            ...
            if self._is_exec and code.strip():
                ...
                self._append_main_code(
                    code_indent + "if not self.is_alive:", indent + 2
                )
    ...
    def _handle_set_end_level(self, block, indent=0, arg_map=None, **kwargs):
        ...
        if self.axis_type[0] == 5:
            self._append_main_code(
                "angle = -(current_angle[1] + current_angle[2])", indent + 2
            )
            self._append_main_code(
            ...
            self._append_main_code(
                "angle_5 = {}".format(
                    ...
                    if self.axis_type[1] == 9 or self.axis_type[1] == 12
                    else "-(current_angle[1] + current_angle[2])"
                ),
                indent + 2,
            )
            self._append_main_code(
                "angles = [*current_angle[:3],0,angle_5,current_angle[5]]", indent + 2
            )
            self._append_main_code(
                "code = self._arm.set_servo_angle(angle=angles)", indent + 2
            )
        elif self.axis_type[0] == 7:
            self._append_main_code(
                "code, ret = self._arm.get_inverse_kinematics(pose=[*current_angle[:3], 180, 0, current_angle[5]])",
                indent + 2,
            )
            self._append_main_code(
                "if not self._check_code(code, 'set_end_level'):", indent + 2
            )
            self._append_main_code("    return", indent + 2)
            self._append_main_code(
                "code = self._arm.set_servo_angle(angle=ret)", indent + 2
            )
        self._append_main_code(
            "if not self._check_code(code, 'set_end_level'):", indent + 2
        )
