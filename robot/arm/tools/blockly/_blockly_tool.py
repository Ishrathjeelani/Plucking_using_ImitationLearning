...
class BlocklyTool(_BlocklyHandler):
    ...
    def to_python(
        ...
        **kwargs
        ...
        if not self._is_converted:
            ...
            self._init_robot_main_class_codes(
                init=init,
    ...
    def _finish_robot_main_run_codes(self, error_exit=True, stop_exit=True):
        ...
        self._append_main_code(
            "            self.pprint('MainException: {}'.format(e))", indent=-1
        )
        ...
        if (
            self._listen_tgpio_digital
            or self._listen_tgpio_analog
            or self._listen_cgpio_state
            or len(self._tgpio_digital_callbacks)
            or len(self._tgpio_analog_callbacks)
            or len(self._cgpio_digital_callbacks)
            or len(self._cgpio_analog_callbacks)
        ):
    ...
    def _init_robot_main_class_codes(
        self,
        init=True,
        wait_seconds=1,
        mode=0,
        ...
        self._append_main_init_code(
            "        self._vars = {}".format(
                {var: 0 for var in self._parse_variables()}
            )
        )
        if len(self._funcs):
            ...
            for key, val in self._funcs.items():
                self._append_main_init_code(
                    "            {}: self.{},".format(
                        json.dumps(key, ensure_ascii=False), val
                    )
                )
            self._append_main_init_code("        }")
        else:
        ...
        if (
            len(self._tgpio_digital_callbacks)
            or len(self._tgpio_analog_callbacks)
            or len(self._cgpio_digital_callbacks)
            or len(self._cgpio_analog_callbacks)
        ):
    ...
    def __define_cgpio_digitals_is_matchs_bin_func(self):
        ...
        if self._define_bin_matchs_func:
            self._append_main_init_code(
                "    def _cgpio_digitals_is_matchs_bin(self, bin_val):"
            )
    ...
    def __define_listen_gpio_thread_func(self):
        ...
        if (
            ...
            if self._listen_tgpio_digital or len(self._tgpio_digital_callbacks):
                self._append_main_init_code(
                    "        _, values = self._arm.get_tgpio_digital()"
                )
                self._append_main_init_code(
                    "        tgpio_digitals = values if _ == 0 else [0] * 2"
                )
            if self._listen_tgpio_analog or len(self._tgpio_analog_callbacks):
                self._append_main_init_code(
                    "        _, values = self._arm.get_tgpio_analog()"
                )
                self._append_main_init_code(
                    "        tgpio_analogs = values if _ == 0 else [0] * 2"
                )
            if (
                self._listen_cgpio_state
                or len(self._cgpio_digital_callbacks)
            ...
            if self._listen_cgpio_state or len(self._cgpio_analog_callbacks):
                self._append_main_init_code(
                    "        cgpio_analogs = [values[6], values[7]] if _ == 0 else [0] * 2"
                )
            ...
            if self._listen_tgpio_digital or len(self._tgpio_digital_callbacks):
                ...
                self._append_main_init_code(
                    "            if _ == 0 and tgpio_digitals is not None:"
                )
                self._append_main_init_code(
                    "                for item in self._tgpio_digital_callbacks:"
                )
                ...
                self._append_main_init_code(
                    "            tgpio_digitals = values if _ == 0 else tgpio_digitals"
                )
            if self._listen_tgpio_digital or len(self._tgpio_analog_callbacks):
                ...
                self._append_main_init_code(
                    "            if _ == 0 and tgpio_analogs is not None:"
                )
                self._append_main_init_code(
                    "                for item in self._tgpio_analog_callbacks:"
                )
                self._append_main_init_code("                    for io in range(2):")
                self._append_main_init_code(
                ...
                self._append_main_init_code(
                    "            tgpio_analogs = values if _ == 0 else tgpio_analogs"
                )

            if (
                not self._listen_cgpio_state
                and len(self._cgpio_digital_callbacks) == 0
                and len(self._cgpio_analog_callbacks) == 0
            ):
            ...
            if self._listen_cgpio_state or len(self._cgpio_analog_callbacks):
                ...
                self._append_main_init_code(
                    "                for item in self._cgpio_analog_callbacks:"
                )
                self._append_main_init_code(
                    "                    for io in range(len(analogs)):"
                )
                self._append_main_init_code(
    ...
    def _init_main_codes(self, arm=None):
        ...
        if arm is None:
            self._append_main_code(
                "arm = XArmAPI(sys.argv[1], baud_checkset=False)", indent=indent
            )
        elif isinstance(arm, str):
            self._append_main_code(
                "arm = XArmAPI('{}', baud_checkset=False)".format(arm), indent=indent
            )
