...
class TransformerTactileVisionModel(pl.LightningModule):
    ...
    def __init__(self, cfg, input_dim, output_dim, input_timesteps, output_timesteps, d_model=128, num_heads=8, num_layers=4):
        super(TransformerTactileVisionModel, self).__init__()
        ...
        # Feature extractor for camera and tactile images
    ...
    def on_train_start(self):
        # Explicitly ensure ResNet is in evaluation mode when training starts
        self.resnet_feature_extractor.eval()

    def forward(self, x, image_data_wc, image_data_wd, image_data_ec, image_data_ed, digit_index, digit_thumb):
    ...
    def training_step(self, batch, batch_idx):
        x, y_states, image_data_wc, image_data_wd, image_data_ec, image_data_ed, digit_index, digit_thumb = batch
        predicted_states = self(x, image_data_wc, image_data_wd, image_data_ec, image_data_ed, digit_index, digit_thumb)
...
def main(cfg:DictConfig):
    ...
    # Save model weights
    trainer.save_checkpoint("tactileVisionProprioTrans_model.ckpt")
    print("Model weights saved as 'tactileVisionProprioTrans_model.ckpt'")
