...
checkpoint_callback = ModelCheckpoint(
    ...
    filename="tactileProprioACT_model",     # Name of the file
...
class ActTactileImagesModel(pl.LightningModule):
    """Transformer Model to predict next states and process status"""
    def __init__(self,cfg, input_dim, output_dim, input_timesteps, output_timesteps, d_model=128, num_heads=8, num_layers=4, action_dim=15):
        super(ActTactileImagesModel, self).__init__()
        ...
        # ResNet feature extractor for individual images (each of the 10 images per batch)
    ...
    def forward(self, x, actions, image_data_wc, image_data_wd, image_data_ec, image_data_ed, digit_index, digit_thumb):
        ...
        x = torch.cat((x, image_features_wc, image_features_wd, image_features_ec, image_features_ed, tactile_features), dim=-1)  # Concatenate along the feature dimension
    ...
    def training_step(self, batch, batch_idx):
        ...
        predicted_states = self(x[:,:, input_indices],x[:,5:,action_indices], images_wc, images_wd, images_ec, images_ed, digit_index, digit_thumb)#[:, input_indices][:,action_indices]
...
def main(cfg:DictConfig):
    ...
    val_dataset = torch.load('TactileVision_val_dataset.pth', weights_only=False)
    test_dataset = torch.load('TactileVision_test_dataset.pth', weights_only=False)
    ...
    trainer = pl.Trainer(logger=logger, max_epochs=20, accelerator='gpu' if torch.cuda.is_available() else 'cpu')#, callbacks=[early_stopping, checkpoint_callback]
    trainer.fit(model, train_loader, val_loader)
    trainer.test(model, test_loader)
    # Save model weights
    trainer.save_checkpoint("tactileVisionProprioACT_model.ckpt")
    print("Model weights saved as 'tactileVisionProprioACT_model.ckpt'")
