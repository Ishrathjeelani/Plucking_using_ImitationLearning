...
class ACTImagesModel(pl.LightningModule):
    ...
    def __init__(self, input_dim, output_dim, input_timesteps, output_timesteps, d_model=128, num_heads=8, num_layers=4, action_dim=15):
        super(ACTImagesModel, self).__init__()
        ...
        # Feature extractors for camera images
    ...
    def forward(self, x, actions, image_data_wc, image_data_wd, image_data_ec, image_data_ed):
        ...
        image_features_wc = torch.cat(image_features_wc, dim=1) 
    ...
    def training_step(self, batch, batch_idx):
        ...
        predicted_states = self(x[:,:, input_indices],x[:,5:,action_indices], images_wc, images_wd, images_ec, images_ed)#[:, input_indices][:,action_indices]
...
# model = ACTImagesModel(input_dim=len(input_columns) + 32*4,  # 2048 features from ResNet50
...
# # Save model weights
# trainer.save_checkpoint("visionProprioACT_model.ckpt")
# print("Model weights saved as 'visionProprioACT_model.ckpt'")
