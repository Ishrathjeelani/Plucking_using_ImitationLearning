...
class TransformerImagesModel(pl.LightningModule):
    """Transformer Model to predict next states and process status"""
    def __init__(self, input_dim, output_dim, input_timesteps, output_timesteps, d_model=128, num_heads=8, num_layers=4):
        super(TransformerImagesModel, self).__init__()
    ...
    def forward(self, x, image_data_wc, image_data_wd, image_data_ec, image_data_ed):
        ...
        x = torch.cat((x, image_features_wc, image_features_wd, image_features_ec, image_features_ed), dim=-1)  # Concatenate along the feature dimension
    ...
    def training_step(self, batch, batch_idx):
        ...
        predicted_states = self(x, images_wc, images_wd, images_ec, images_ed)
...
# # Save model weights
# trainer.save_checkpoint("visionProprioTrans_model.ckpt")
# print("Model weights saved as 'visionProprioTrans_model.ckpt'")
